...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    Figure \ref{BioIsop:results:emissions:fig_time_series_vs_megan} shows a priori emissions over Australia and the a posteriori emissions calculated using OMI.
    The figure shows the time series (daily midday averages) their absolute differences (a priori - a posteriori) over 2005 TODO: Update plot to MYA.
    ...
    Additionally, in the northern region, the a posteriori does not show significant emissions from mid March through to November which are seen in a priori emissions.
    This overestimate by the a priori may be caused by some mixture of overly high emission factors and high sensitivity to temperatures during the dry season.
    ...
    This figure is repeated using monthly medians (of the daily midday estimates) in Figure \ref{BioIsop:results:emissions:fig_time_series_vs_megan_monthly} over 8 years from 2005 to 2012.
    ...
    %JENNY NOTE: Need to expand on this a bit more - you are suggesting that the a priori isn't actually better in summer, you just have less data to constrain the a posteriori? Can you show that? Is that what TODO Figure in the next para will do?
    
    % TODO: Update to remove 'Days since yyyymmdd labels', change to multiyear timeseries averages instead of 1 year, title to posteriori
    ...
    Figure \ref{BioIsop:results:emissions:fig_megan_vs_Enew_regional_mya} shows the multi-year monthly mean and inter-quartile range of daily midday isoprene emissions estimates, averaged regionally. 
    Months outside of May to August show the a posteriori lower than the a priori.
    The most overlap is seen in the south-eastern region, and both eastern regions show the highest variance, potentially due to diversity within the regions and anthropogenic influence.
    ...
    There is only weak correlation between daily estimates, and the distributions are quite different with more variance shown by the a priori.
    This is likely due to the filtering applied to satellite data (e.g., whenever cloud coverage exceeds 40\%) which reduces the number of days of a posteriori emission calculations.
    ...
    The portion of this correlation due to seasonality is examined by re-running the regression after deseasonalising both data sets by subtracting the multi-year monthly average from each dataset.
    ...
    \mypicw{\textwidth, totalheight=0.8\textheight}{Figures/OMI_link/Emiss/monthly_Egressions.png}{%
  ...
  \label{BioIsop:results:new_emiss}
    ...
    This box is typical of our south-eastern region and is not heavily impacted by the anthropogenic filter.
    ...
    \subsubsection{Implications for HCHO}
      ...
      The remaining differences are due to temporal averaging of the applied scaling factor $\alpha$.
      ...
      I also examine the effects of scaling isoprene emissions on the correlation and variances between modelled and satellite based HCHO columns.
      Figure TODO: shows the regressions between $\Ogc$, $\Ogca$, and $\Oomi$, for summer and winter between for each region.
      ...
      Figure TODO shows variances in each season for each region in $\Ogc$, $\Ogca$, and $\Oomi$.
      TODO: Analysis of plots
    ...
    \label{BioIsop:results:new_emiss:trends}
      ...
      % JENNNY NOTE: I don't get much out of these figures, and they don't really show whether or not there is a quantifiable change from a priori to a posteriori. I'd suggest keeping the isoprene one (3.20) as an example of the process, but then using either a table or a barchart to show the trends for the rest.
      % JENNY NOTES:If a table, you would have a column for a priori trend and a posteriori trend with a significance marker for each and then rows for each species and region (I would probably group by species then list each region so e.g. 
      % Isoprene
      % SE Aus   XX    YY
      % N Aus      XX   YY
      % etc. then repeated for HCHO, etc.
      % JENNY NOTES: If a barchart, I am picturing a bar showing the trend for a priori & a posteriori next to each other for each region, with a significance marker above. One bar chart per species perhaps, with different regions along the x-axis. Let me know if unclear.
...
%    \label{BioIsop:uncertainty:summary}
  ...
  We identify the uncertainties in the linear regression used to relate HCHO to isoprene emissions, as well as in the satellite data product used as the basis for the inversion; however, these uncertainties lack verification against measurements.
  Even as this top-down inversion attempts to work around the lack of measurement over Australia, it suffers from the lack of independent observations against which it can be verified.
  ...
  Reliable OMI measurements are less frequent (so uncertainty is higher) at high solar zenith angles, which occurs at higher latitudes and during winter.
  ...
  The process (or methods) used to calculate each of these uncertainties is described in the following subsections.
  ...
  \subsection{Top down emissions}
    ...
    Uncertainty in terms $\Oomi$, and $\Omega_{OMI,0}$ come from the OMHCHO slant columns ($SC$), the background error, and the $AMF$ calculated to transform them into vertical columns ($\Omega = \frac{SC}{AMF}$ from equation \ref{Model:AMF:eqn_AMFFrac}).
  ...
  \subsection{Satellite Uncertainty}
    ...
    % JENNY NOTE: You say there are three things but then you follow with two sub-sections that have different titles than the three things listed here. Can you make these match such that Section 3.4.3.1 is fitting error, etc.? If you need further sections as it looks like you have below then add them to this discussion.
    % TODO: Match these items to the following subsubsections
    ...
    \end{enumerate}
    %
    % JENNY NOTE: I am finding this whole section VERY confusing. There is a lot of redundancy, jumping back and forth between things that are (I think?) the same or at least related, use of different terminology for the same/similar things...
    % JENNY NOTE contd.: I think you need to think carefully about the structure of this section, and lay things out one by one, providing all the detail for each type of uncertainty before moving on to the next. Make it very clear for the reader how things are (or aren't) related)...
    %
    Fitting error is provided by the OMI product and represents the uncertainty in the DOAS technique used to estimate HCHO concentrations.
    This source of error can be reduced through spatial and temporal averaging.
    ...
    Here we use 30\% as a rough estimate of error in this term (TODO: cite, this is estimated by Palmer somewhere), since measurements over Australia are lacking.
    ...
    Since we expect oceanic background HCHO to be invariant, variance in remote ocean HCHO can be used as a rough estimate of background uncertainty. TODO: IS THIS ESTIMATE OF OVERALL UNCERTAINTY OR JUST BACKGROUND?
    ...
    For simplicity, uncertainty from the HCHO background is assumed.
    Further simplification is made by assuming that this background error does not change when calculating a new reference sector correction ($RSC$, see Section \ref{Model:omiRecalc:RSC}).
    ...
    Uncertainty introduced through AMF calculation and reference sector correction ($\Omega_0$) needs to be additionally determined to give a representation of the confidence in vertical column amounts.
    ...
    This is considered to be the error in the slant column ($\Delta SC$), which is combined with our assumed relative AMF error ($\frac{\Delta AMF}{AMF}$) of 30\%, and background error.
    $\Delta \Oomi$ and $\Delta \Omega_{OMI,0}$ are calculated for Equation \ref{BioIsop:uncertainty:eomi:eqn_delta_apost} using the quadrature rules as follows:
    \begin{align}
      \Omega  & = \frac{SC - RSC}{AMF} \notag \\
      \Delta (SC-RSC) & = (SC-RSC) \sqrt{(\frac{\Delta SC}{SC})^2 + (0.3)^2} \notag \\
      \Delta \Omega & = \Omega \sqrt{ ( \frac{\Delta (SC-RSC)}{(SC-RSC)} )^2 + ( 0.3 )^2 } 
    \end{align}
    The $RSC$ term comes from Chapter \ref{Model} Equation \ref{Model:omiRecalc:vcc:eqn_vcc} for calculating the corrected $\Omega$.
    As a simplification, uncertainty in the $RSC$ term is assumed to be 30\%.
    This is based on the 15\% background error assumed in \textcite{Curci2010} following \parencite{Dufour2009}.
    ...
    % TODO: This stuff will be combined into with the First paragraph
    ...
    One method of finding overall (TODO: IS THIS ESTIMATE OF OVERALL UNCERTAINTY OR JUST BACKGROUND?) satellite uncertainty involves using the variance over the remote Pacific (15\degr S to 15\degr N, and 180\degr W to 120\degr W) \parencite[][]{DeSmedt2012,DeSmedt2015}.
    ...
    % satellite biases:
    OMHCHO has been seen to underestimate observed HCHO by up to 40\%, although this amount is regionally dependent \parencite{Zhu2016,DeSmedt2015,Barkley2013}).
    TODO: list regions and their overestimates.
    ...
    We note therefore that our product may be biased by up to 40\% due to satellite bias.
    ...
    Since we lack sufficient suitable measurements to estimate satellite bias over Australia, we assume bounds on potential satellite underestimation of up to 40\%.
...
\section{Conclusions and implications}
  ...
  % JENNY NOTE: It should go into a discussion somewhere : the REASON for the decreases you find is not really clear. Is it the emission factors (cf Emmerson 2016)? Is it the lack of accounting for soil moisture (cf Sindelarova)? This should be a cohesive discussion, that brings the different threads from the literature together and postulates how one might further improve GEOS-Chem in futre.
  
  Global and Australian emissions estimates for isoprene still range widely due to the emission models sensitivities to inexact parameters including leaf area indices, plant functional type emission factors, and meteorological factors, with the full story behind the decrease seen here in a posteriori emissions not yet clear.
  It is likely a mixture of poorly defined emission factors \parencite[e.g.][]{Emmerson2016}, unaccounted for soil moisture \parencite[e.g.][]{Sindelarova2014} and poorly understood responses to meteorological stresses in Australian forests (TODO: cite papers talking about clay and poor moisture responses).

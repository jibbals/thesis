...
\section{Results}
  ...
  \subsection{Updated emissions}
    ...
    % UP TO HERE JENNY
    % UPDATE: fix Y axis labels -> a priori, X axis -> a postiori, add units, remove negative axes
    % Figure from !?!?! TODO
    \mypic{Figures/OMI_link/Emiss/monthly_Egressions.png}{%
      ...
      % JENNY COMMENT:So - each dot in the figure is one gridbox and one month (e.g. Jan) that has been averaged over all years? So if there were 20 grid boxes in the region you would end up with 60 points? Is that right? I think a bit of extra wording is needed for clarity.
      Coloured by regions shown in Figure \ref{BioIsop:results:fig_subregions}.
      Linear best fit regression is shown with line equation inset along with regression coefficient.

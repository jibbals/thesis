...
\label{BioIsop:intro}  
  ...
  \subsection{Top-down emissions estimates}
    ...
    \subsubsection{Linear}
      ...
      The top down estimation process in this thesis is explained and detailed in chapter \ref{BioIsop}.
...
\section{Methods}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \item
        ...
        This value is set to 4000 in \textcite{Marais2014}, based on local NO$_X$, windspeeds, and emission concentrations. (TODO: determine properly for Aus).
  ...
  \subsection{Calculating modelled slope}
    ...
    Estimates of $Y_i$ or $slope$ can be attained through modelling \parencite[e.g.][]{Millet2006}.
    ...
    Some affects from the low emissions in grid boxes which are largely oceanic can be seen and are handled by smearing filters applied in section \ref{BioIsop:Method:Smearing}.
    ...
    Modelled background HCHO concentrations are used for comparison with other datasets, along with creating the reference sector correction for satellite measurements.
  ...
  \subsection{Calculation of Emissions}
    ...
    Emissions of precursors are easy to calculate using the $slope$ calculated in the prior section from modelled HCHO and isoprene columns:
    \begin{equation}
    \Omega_{HCHO} = slope \times E_isop + \Omega_0
    \end{equation}
  ...
  \subsection{Accounting for smearing}
    ...
    In this work a run of GEOS-Chem using globally halved isoprene emissions (with no other changes) is performed to test and filter smearing.
    ...
    This leads to the method used here and elsewhere: testing the correlation between HCHO enhancement and isoprene emissions does not change too much between runs.
    This is done through assuming the correlations are equivalent and then finding where the assumption leads to unlikely yields.
    ...
    The calculation of smearing is closely tied to isoprene to HCHO yield and modelled regression slopes.
    From equation \ref{BioIsop:Method:slope:eqn_isop_to_hcho} we have modelled slope (S) being the yield of HCHO per C of emitted isoprene divided by HCHO loss rate per second ($S = \frac{Y_{isop}}{k_{HCHO}}$).
    ...
    \end{eqnarray}
    There are several assumptions which need to be understood, as these are what is actually tested by the smearing calculation.
    The initial assumption is that the system is in a steady state, with no transport of isoprene affecting HCHO columns, this is the basis for the equations in \ref{BioIsop:Method:Smearing:calculation:eqn_runs}.
    ...
    The modelled slope multiplied by the column HCHO loss rate ($k_{HCHO}$) should approximate the HCHO yield from isoprene \parencite{Palmer2003, Barkley2013}.
    %TODO: find lifetime and yields from GEOS-Chem
    Using the mean HCHO lifetime of Z hours applied against $\Delta S$ from 1000 to 4000 corresponds to yields in the range of X to Y HCHO per C
    ...
    TODO: Determine and explain choice of $\hat{S}$ range and why we expect yields and loss rates around these levels.
    ...
    %TODO: Figure scatter plot of $\hat{S}$ vs $\Omega_{NO_2}$) shows how smearing is related to NO$_X$ over Australia.
    %TODO: Figure same but binned by nox columns at about 0.1e15 width, over a longer time)
    ...
    \subsubsection{NOx dependence}
      ...
      NO$_2$ measured by OMNO2d gives us a daily mid-day measurement which we can compare to output from GEOS-Chem to determine how well the model does at simulating NO$_2$ (presented back in section \ref{Model:Analysis:NOx}).
      %\textcite{Travis2016} show how NO$_X$ can be used to examine model bias in ozone (potentially due to NO$_2$ bias) over the USA.
      The affect of NO$_2$ on smearing can be seen in figure TODO
      This plot shows how smearing over Australia compares to satellite NO$_2$ levels, with smearing distributions binned by NO$_2$.
...
\section{Results}
  ...
  \subsection{Emissions comparisons}
    ...
    They calculate emissions which create the closest match between model and satellite vertical columns, and compare these postiori data with the a priori (satellite data) and independent data sets.
    ...
    By scaling up satellite HCHO by 40\% we increase our isoprene emissions estimates by the same, as our estimate is proportional to satellite HCHO.

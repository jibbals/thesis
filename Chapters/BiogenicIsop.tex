...
\section{Methods}
  ...
  \subsection{Accounting for smearing}
    ...
    Using the mean HCHO lifetime of $\sim 2.5$ hours applied against $\Delta S$ from 1000 to 4000 corresponds to yields in the range of X to Y HCHO per C.
    ...
    This is from the equation relating the lifetime to mass and loss rates: 
    \begin{equation*}
      \tau    = [HCHO] / L
    \end{equation*}
    For each grid square over Australia the lifetime over the year is shown in figure TODO, one colour for each gridbox is shown over time.
    Since this is based on the daily average HCHO loss, the daytime lifetime of HCHO is approximately halved ranging from $\sim 1.5$-$4$~hrs in Summer and X to Y in winter.
    ...
    \subsubsection{NOx dependence}
      ...
      \mypic{Figures/OMI_link/Filters/smearing_nox.png}{%

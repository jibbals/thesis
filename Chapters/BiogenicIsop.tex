...
\section{Methods}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \item 
        $\Omega$ are used to create a top-down estimate of biogenic isoprene emissions ($E_{new}$~atoms C cm$^{-2}$ s$^{-1}$)
      \begin{enumerate}
        \item 
  ...
  \subsection{Calculation of Emissions}
    ...
    This leaves $E_{new}$ as the only unknown once the satellite measurements are processed to match the temporal and horizontal resolution of $S$.
    ...
    When calculating the $E_{isop}$ from our modelled slope, negative emissions result when using OMI measured columns are lower than the background amounts (as $E_{new} = \frac{\Omega - \Omega_0}{S}$).
  ...
  \subsection{Accounting for smearing}
    ...
    \subsubsection{Calculation of smearing}
      ...
      This figure highlights the seasonal nature of HCHO loss rates, although midday numbers are expected to be less extreme.
...
\label{BioIsop:Uncertainty}
  ...
  In order to quantify uncertainty in $\Delta{E_{new}}$ we need the uncertainties in the underlying terms: $\Delta{S}$, $\Delta{\Omega}$, and $\Delta{\Omega_0}$. 

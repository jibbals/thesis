...
\section{Results}
  ...
  \subsection{Emissions comparisons}
    ...
    \mypic{Figures/OMI_link/Emiss/tga_map.png}{%
      Top row: multiyear mean emissions in \tgpyr from GEOS-Chem (running MEGAN) $E_{GC}$ and top-down emissions (assuming sinusoidal daily cycle: see section \ref{BioIsop:Method:calculation}) respectively.
    ...
    \begin{table}\begin{threeparttable}
      ...
      \begin{tabular}{ l  c  l } 
        ...
        \midrule
        43(2) & 445(18) & GEOS-Chem 2005-2012 \\
        19(2) &  & top-down 2005-2012 \\
        \bottomrule
      \end{tabular}
      %\begin{tablenotes} 
      %  \item a: 
  ...
  \subsection{Comparison with measurements}
    ...
    Comparison between ground-based measurements and large (2x2.5\degr) averaged grid squares suffers from representational error.
    Due to high uncertainty in components of the top-down emissions estimate, temporal resolution is also limited.
    MUMBA, SPS1 and SPS2 each provide only a couple of comparable data points.
...
\label{BioIsop:Uncertainty}
  ...
  In order to quantify uncertainty in $\Delta{E_{OMI}}$ we need the uncertainties in the underlying terms: $\Delta{S}$, $\Delta{\Omega}$, and $\Delta{\Omega_0}$. 

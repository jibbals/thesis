...
\label{BioIsop:intro}  
  ...
  Isoprene emissions may be overestimated in Australia since they are based on measurements taken from a few young trees \parencite{Winters2009} which may emit more than older trees \parencite{Emerson2016}.
  The sample of trees include 4 types of Eucalyptus, which are not representative of the hundreds of species which make up Australian forests, and how these species depend on biological and meteorological stresses is unclear \parencite{Winters2009, FortemsCheiney2012}.
  ...
  \subsection{Aims}
    ...
    This a postiori top-down estimate is then compared against sparse available ground-based measurements.
  ...
  \subsection{Existing emissions estimates}
    ...
    In this thesis I prioritise the analysis of a top-down emissions estimate compared against MEGAN, along with flow on effect of changed emissions on modelled ozone levels.
  ...
  \subsection{Top-down isoprene emissions estimates}
    ...
    \subsubsection{Bayesian}
      ...
      In this work I do not use the Bayesian method due to the heavy computational costs involved.
    ...
    \subsubsection{Linear}
      ...
      This allows an inversion using more than 8 years of satellite and model data within a reasonable time frame.
...
\section{Methods}
  ...
  A relationship is modelled between midday column HCHO and isoprene emissions using GEOS-Chem, and then this relationship is applied to satellite measured HCHO total columns to derive a new isoprene emissions estimate.
  Daily modelled values averaged between 13:00-14:00~LT are used to match the overpass time of the Aura satellite.
  Then the slope is taken from the reduced major axis (RMA) regression between the a priori emissions (those from GEOS-Chem, $\apri$) and tropospheric HCHO columns in each model grid box each month.
  %Figure \ref{BioIsop:method:slope:fig_regressions} shows an example; the modelled regression between emissions and tropospheric columns for January, 2005.
  ...
  \label{BioIsop:method:scaled}
    ...
    After creating our a postiori isoprene emissions estimate, we run GEOS-Chem again with biogenic emissions scaled to match the new estimate. 
...
\section{Results}
  ...
  \subsection{Updated emissions}
    ...
    Months outside of May to August show the a postiori lower than the a priori, except in the south eastern region.
  ...
  \label{BioIsop:results:new_emiss}
    ...
    %unscaled GEOS-Chem biogenic emissions are the a priori, top down midday emissions are the a postiori, and emissions from the scaled GEOS-Chem run are denoted E$^{\alpha}_{GC}$.
    ...
    \label{BioIsop:results:new_emiss:trends}
      ...
      Figure \ref{BioIsop:results:new_emiss:trends:fig_emission_anomalies} shows monthly deseasonalised a priori and a postiori midday emission anomalies for each region.
      ...
      \mypic{Figures/OMI_link/new_emiss/trend_O3.png}{%
        A priori (row 1) and a postiori (row 2) surface ozone anomaly from multiyear monthly mean, split by region (see Figure \ref{BioIsop:results:fig_subregions}).
      }{\label{BioIsop:results:new_emiss:trends:fig_ozone_anomalies}}
      
      \mypic{Figures/OMI_link/new_emiss/trend_hcho.png}{%
        A priori (row 1) and a postiori (row 2) surface HCHO anomaly from multiyear monthly mean, split by region (see Figure \ref{BioIsop:results:fig_subregions}).
      }{\label{BioIsop:results:new_emiss:trends:fig_hcho_anomalies}}
      
      \mypic{Figures/OMI_link/new_emiss/trend_NO2.png}{%
        A priori (row 1) and a postiori (row 2) surface NO$_2$ anomaly from multiyear monthly mean, split by region (see Figure \ref{BioIsop:results:fig_subregions}).
...
%    {Top left shows the a postiori emissions, while the top right shows same calculations without applying any fire or smoke filter.

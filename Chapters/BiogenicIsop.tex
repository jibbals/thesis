...
\section{Results}
  ...
  \label{BioIsop:results:new_emiss}
    ...
    %TODO Plot and analysis prior to sending to Jenny
  ...
  \subsection{Trends}
    ...
    The same process is repeated for surface ozone, surface HCHO, and surface NO$_2$, with anomaly series shown in Figures \ref{BioIsop:results:trends:fig_ozone_anomalies} to \ref{BioIsop:results:trends:fig_no2_anomalies}.
    None of these metrics shows any trend over the 8 years either before or after scaling isoprene emissions.
    ...
    }{\label{BioIsop:results:trends:fig_ozone_anomalies}}
    ...
    }{\label{BioIsop:results:trends:fig_hcho_anomalies}}
    ...
    }{\label{BioIsop:results:trends:fig_no2_anomalies}}
    
    
    %Table \ref{BioIsop:results:trends:tab_trend_slopes} shows monthly slopes for ozone, HCHO, and NO$_2$ before and after scaling isoprene emissions, where the regression is significant (Wald test $p$ value $< 0.05$).
    ...
    %  \end{tablenotes}
    %  \label{BioIsop:results:trends:tab_trend_slopes}
    %\end{threeparttable}\end{table}
...
%    \label{BioIsop:uncertainty:summary}
  ...
  \subsection{Satellite Uncertainty}
    ...
    Other literature has found satellite HCHO columns to be up to 40\% too low, however we can not quantify this over Australia due to insufficient measurements.
    Since our a postiori is linearly related to the satellite HCHO, any bias is directly transferred.
    This suggests that our product may be biased by up to 40\% due to un-quantified satellite bias.
    ...
    Validation is important due to the various uncertainties in the satellite remote sensing process, with a priori assumptions having the greatest effect on structural uncertainty between measurements techniques \parencite{Lorente2017}.
    One example is \textcite{Zhu2016}, who used SEAC$^4$RS aircraft HCHO measurements over the southeastern US as model validation. 
    They showed that a bias in the assumed OMI shape factor leads to a bias between satellite and SEAC$^4$RS measurements.
    Since we lack sufficient suitable measurements to estimate this quantity over Australia, we estimate bounds on potential satellite biases based on literature.

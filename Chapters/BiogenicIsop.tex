...
\section{Methods}
  ...
  \subsection{Accounting for smearing}
    ...
    \subsubsection{NO$_x$ dependence}
      ...
      There is also a higher number of data points below the lower threshold before that same NO$_2$ value, showing that transport is a bigger issue at NO$_2 < 5 \times 10^{14} $~molec cm$^{-2}$. 
...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    The difference between a priori and a posteriori estimates differs between regions, but a seasonal overestimate peaking in summer can be seen in all regions.
    ...
    Uncertainty is summarised for the a posteriori using the mean monthly uncertainty per grid square within each region, divided by the square root of how many months are averaged (24, 8 years of 3 months per season).
    ...
    Outside of summer (spring in the Northern region) the overestimation lies within the potential bias caused by satellite underestimations of HCHO. 
    However, the most overestimated season within each region is still outside this wide range of potential bias and uncertainty.
    ...
    When comparing GEOS-Chem (MEGAN) to the a posteriori calculated using our top-down inversion, we find a decrease of from 39\tgpyr ~to 21\tgpyr ~(decrease of $\sim 46\%$).
    ...
    \subsubsection{Diurnal emissions}
      ...
      % TODO: Show a posteriori regional instead of a priori with coloured lines
      % TODO: Remove vertical lines, just state error bar is at 1330LT
  ...
  \label{BioIsop:results:new_emiss}
    ...
    \subsubsection{Implications for ozone}
      ...
      % TODO: find or recreate this image, or improve above image to summarise all the info
...
%      }{\label{BioIsop:results:new_emiss:trends:fig_no2_anomalies}}
  ...
  \subsection{Comparison with in situ measurements}
    ...
    Analysis of this dip could be examined in future work, and would require more analysis of the local and synoptic HCHO patterns.
  ...
  \subsection{Satellite Uncertainty}
    ...
    \begin{description}
      ...
      \item [Uncertainty in AMF calculations] \hfill \\
        ...
        % JENNY NOTE: How does this compare to FTIR comparison?
...
\section{Conclusions and implications}
  ...
  My top down biogenic isoprene emission estimate showed that MEGAN overestimates emissions in summer by up to a factor of 5, and total yearly Australian emissions are reduced from 39\tgpyr ~to 21\tgpyr ~(decrease of $\sim 46\%$).
  ...
  A posteriori uncertainty was shown to be on the order of 50\% in summer (monthly, per grid square), with a potential bias coming from satellite data of $-13\%$ to $40\%$.

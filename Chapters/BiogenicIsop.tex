...
\label{BioIsop:intro}  
  ...
  These react quickly with NO$_X$ to form products such as formaldehyde (HCHO), secondary organic aerosols (SOAs), and ozone (O$_3$).
  ...
  The global emission models used to derive these estimates are based on modelling emissions from different plant species (phenotypes), which are seldom well understood within Australian forests.
  Isoprene emissions may be overestimated since they are based on measurements of taken from a few heavily emitting young eucalyptus trees which are not representative \parencite{Winters2009, FortemsCheiney2012,}.
  ...
  Australian forests are strong emitters of both isoprene and monoterpenes, which go on to form various products including secondary organic aerosols, oxygenated volatile organic compounds (OVOCs), ozone, OH, and HO$_2$.
  ...
  One frequently used model of biogenic VOC (BVOC) emissions is MEGAN \parencite{Guenther2000} which estimates $\sim 1150$\tgpyr.
  ...
  The tropospheric effects of NMVOCs on the hydroxyl radical (OH), O$_3$, SOAs, and methane longevity all interconnect to form a very complex system which still suffers from relatively large uncertainties in both measurement and chemistry mechanisms.
  ...
  Direct isoprene measurements are costly and sparse within Australia, while satellite HCHO data are plentiful and freely available, making this technique very attractive.
  ...
  HCHO is the dominant product of most BVOCs that is measured by remote sensing.
  ...
  This can lead to different estimates between instruments or methodologies as described in \textcite{Lorente2017}, and both validation and comparison is more important when using these remotely sensed data.
  ...
  \subsection{Aims}
    ...
    Estimates remain relatively uncertain globally, however the top-down technique has successfully improved inventories in other countries.
    In this section we introduced isoprene emissions over Australia, and discuss why they are uncertain and how we can improve estimates.
    We describe how emissions may be calculated using satellite datasets.
    Section \ref{BioIsop:Method} describes the technique used to calculate isoprene emissions, and results are examined in Section \ref{BioIsop:Results}. 
    In section \ref{BioIsop:Conclusions} we show how these changes in emissions would affect ozone concentrations in Australia, along with some other chemical processes.
    ...
    These estimates are compared to several campaigns (SPS1, SPS2, and MUMBA, see section \ref{Model:Datasets}).
    Sensitivity to satellite AMF calculation is examined and quantified for some scenarios.
...
\section{Methods}
  ...
  \subsection{Accounting for smearing}
    ...
    This figure highlights the seasonal nature of HCHO loss rates, although midday numbers are expected to be less extreme. %(todo: rough numbers from caaba mecca).
    ...
    \subsubsection{NOx dependence}
      ...
      Conversions between HCHO per unit C yield and molar \% yield from species X given by the equation $ Y_{molar \%} = 100 \times C_X \times Y_{HCHO \,C^{-1}}$, where $C_X$ is how many carbon are within species X (5 for isoprene).
...
\section{Results}
  ...
  Australia is a large country with heterogeneous environmental conditions.
  ...
  \subsection{HCHO Products and yield}
    ...
    Figure \ref{BioIsop:Results:HCHOYield:fig_Yield_Lifetime}: shows the GEOS-Chem Y$_{isop}$ and lifetime of HCHO estimations throughout the year.
    ...
    The noise in the southwest region may be indicative of heavy filtering and due to westerly winds, which can lead to both smearing and transported pollution.
...
\label{BioIsop:Uncertainty}
  ...
  \subsection{Satellite Uncertainty}
    ...
    If we infer from this that there is 40\% bias and random uncertainty remains unchanged, then in the S term, we find TODO increase or decrease in $\Delta{E_{new}}$ of some amount through changing $\Delta{S}=$ .

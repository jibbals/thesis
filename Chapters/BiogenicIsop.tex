...
\section{Methods}
  ...
  \subsection{Calculation of modelled slope}
    ...
    Days where smearing is expected to affecting local levels of HCHO are removed before calculating $S$, and a quick analysis is performed on how the filter affects monthly slope, correlation, and uncertainty.
    Figure \ref{BioIsop:method:slope:fig_smear_filter_prior} shows the calculated slope for 2005-2012, along with its 95\% confidence interval for the 2\degr x 2.5\degr ~grid box containing Sydney Sydney.
    ...
    %When limiting smearing ($\hat{S}$) to within 800-4600~s, %GEOS-Chem correlations between isoprene emissions and HCHO columns improve marginally and not uniformly.
    ...
    % JENNY TODO: This section is in the wrong place. Very repetitive of what is already there at the end of 3.2.5 (calculation of modelled slope) and Figure 3.3 which you've already discussed. Move this text up to that section and condense so that you aren't duplicating the discussion (especially the second paragraph of this one). Move Figure 3.6 along with it.
...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    \mypicwh{\textwidth}{0.8\textheight}{Figures/OMI_link/Emiss/monthly_Egressions_final.png}{%
      Scatter plot of a priori emissions against a posteriori emissions using monthly averaged grid squares as regression datapoints.
    ...
    Our a posteriori estimate of 19\tgpyr ~suggests isoprene emissions may be lower than all prior bottom up estimates, and is close to the lowest top-down estimate of \textcite{Bauwens2016} of 26 \tgpyr.
  ...
  \subsection{Modelled impacts of reduced isoprene emissions}
    ...
    Figure \ref{BioIsop:results:new_emiss:fig_alpha_mya_series} shows $\alpha$ for January and June averaged over 2005-2012, along with the time series of $\apri$ and $\apost$ and $\alpha$ calculated for the 2\degr x 2.5\degr ~grid box containing Sydney and their multi-year seasonal average.
    ...
    \mypic{Figures/OMI_link/new_emiss/alpha_mya.png}{%
      ...
      %Row 3: Monthly averages of the same terms in Row 2.
      TODO: Fix up colour bars, remove row 3.
    }{\label{BioIsop:results:new_emiss:fig_alpha_mya_series}}
    ...
    \subsubsection{Implications for HCHO}
      ...
      \mypic{Figures/OMI_link/new_emiss/HCHO_total_columns_seasonal.png}{%
        Total column HCHO before (left) and after (middle) scaling isoprene emissions, and their relative differences (right).
      ...
      % Figure from chapter_3_isop.hcho_summary
      ...
      \mypicw{0.5\textwidth}{Figures/OMI_link/check_GC_profile.png}{%
        Monthly averaged HCHO profile over Wollongong modelled by GEOS-Chem before (VMR) and after (VMR$^{\alpha}$) scaling isoprene emissions.
    ...
    \subsubsection{Implications for ozone}
      ...
      \mypic{Figures/OMI_link/new_emiss/O3_surf_map_seasonal.png}{%
        Surface (up to $\sim{150}$~m) ozone before (left) and after (middle) scaling isoprene emissions, and their relative differences (right).
        Top row shows summer (DJF) averaged total columns, while bottom row shows the winter (JJA).
...
%    
    ...
    % Figure from where????
    % TODO: add monthly time series? maybe small subplot at top?
    \mypic{Figures/OMI_link/FTIR_TC_Comparison_MYA.png}{%
      Multi-year (from all days where both measurements and model data exist) monthly mean total column ($\Omega$) HCHO from the FTIR instrument, and the colocated convolved GEOS-Chem equivalent before ($\Omega_{GC}$) and after ($\Omega_{GC}^{\alpha}$) scaling isoprene emissions.
  ...
  \subsection{Satellite Uncertainty}
    ...
    \begin{description}
      ...
      \item [Uncertainty in AMF calculations] \hfill \\
        Air mass factors model vertical sensitivity to slant path measurements in the satellite instruments, and uncertainties arise predominantly from uncertain cloud parameters \parencite{Palmer2006}.

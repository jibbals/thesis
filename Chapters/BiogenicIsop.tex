...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    Figure \ref{BioIsop:results:emissions:fig_regional_seasonal_comparison} shows the multi-year seasonal emissions for each region for the a priori and a posteriori emissions.
    The seasonal cycle and the difference between the two metrics on the order of twice as high in the a priori.
    Absolute differences are highest in spring and summer, when emissions are generally greatest.
    TODO: Check if this is because of ocean zeroes: Additionally, in the northern region, the a posteriori does not show significant emissions from mid March through to November which are seen in a priori emissions.
...
%    \label{BioIsop:uncertainty:summary}
  ...
  \begin{table}\begin{threeparttable}
    ...
    Figure TODO shows the average seasonal relative errors over Australia, along with a time series of total emissions and the error bars using monthly averaged data and MEGAN emissions for reference.
  ...
  \subsection{Top down emissions}
    ...
    For $\Delta \Oomi$ and $\Delta \Omega_{OMI,0}$, uncertainty comes from instrument, model, and background correction terms and is described and calculated in Section \ref{BioIsop:uncertainty:satellite}.
    
    TODO: summary of uncertainty seasonally and regionally in the a posteriori.
  ...
  \subsection{Model Uncertainty}
    ...
    \begin{equation}
      \frac{\Delta S}{S} = (\frac{CI_{UB}}{S} - 1) \times 100
    \end{equation}
    ...
    For example, if the interval upper bound is 30\% higher than the slope, relative uncertainty is set to 30\%.
  ...
  \subsection{Satellite Uncertainty}
    ...
    \begin{description}
      ...
      \item [Uncertainty of HCHO background] \hfill \\
        ...
        In this work vertical columns are corrected using reference sector measure ments combined with modelled HCHO, which is described in Chapter \ref{Model} (Section \ref{Model:omiRecalc:RSC}).
    ...
    Figure \ref{BioIsop:uncertainty:satellite:fig_pixel_count_summary} shows the mean daily pixel counts per \lowhr ~ grid square in each region.
    Winter has lower pixel count for all regions except the north-east, where summer storms and increased cloudiness lead to reduced satellite measurements.
    ...
    \mypic{Figures/OMI_link/pixel_count_barchart.png}{%
      Multi-year seasonally averaged mean daily pixel count for each region, with standard deviations shown by vertical error bars.
    }{\label{BioIsop:uncertainty:satellite:fig_pixel_count_summary}}
    
    Fitting error is the error in the slant column ($\Delta SC$), which is combined with our assumed relative AMF error ($\frac{\Delta AMF}{AMF}$) of 30\%, and background error.
    $\Delta \Oomi$ (and $\Delta \Omega_{OMI,0}$) are calculated %for Equation \ref{BioIsop:uncertainty:eomi:eqn_delta_apost}
    using the quadrature rules as follows:
    \begin{align}
      \Omega          & = \frac{SC - RSC}{AMF} \notag \\
    ...
    \subsubsection{Uncertainty of HCHO background}
      ...
      The $RSC$, or background correction, is based on differences in the remote Pacific between HCHO slant columns measured by OMI and monthly averages from GEOS-Chem.
      ...
      For each month, the $\Delta RSC$ is set to the standard deviation of the RSC over Australian latitudes ($45$\degr S to $10$\degr S) in all tracks.

...
\label{BioIsop:intro}  
  ...
  Australian isoprene emissions are poorly understood due to poor measurement coverage.
  The lack of measurements makes it difficult to estimate important subsequent processes such as ozone and secondary organic aerosol (SOA) formation, and OH radical cycling.
  ...
  Emission factors are combined with meteorological parameters to determine emissions in simulations.
  ...
  These react quickly with NO$_X$ to form oxygenated VOCs (OVOCs), SOAs, and ozone.
  ...
  This leads to different estimates between instruments as described in \textcite{Lorente2017}, which means validation and comparison is more important when using these remotely sensed data.
  In this thesis HCHO from the OMI instrument (\ref{Model:omhcho}) is used as the basis for top-down isoprene emissions estimation over Australia.
  ...
  \subsection{Aims}
    ...
    This work aims to improve isoprene emission estimates in Australia.
    ...
    A secondary aim is to determine whether this top-down estimate improves model fit to the sparse available ground-based measurements.
    This is achieved through an examination of GEOS-Chem model output after updating isoprene emissions which are used as inputs.
  ...
  \subsection{Top-down isoprene emissions estimates}
    ...
    This is the idea behind using HCHO enhancement over continents to determine NMVOC emissions.
...
\section{Methods}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \end{enumerate}
      \item
        Modelled slope calculations depend on several assumptions which are not always valid.
  ...
  \subsection{Calculating modelled slope}
    ...
    Initially studies assumed a simple linear steady-state relationship between HCHO and its precursors \parencite{Palmer2003, Palmer2006, Millet2006}.
    ...
    In this thesis modelled background columns are calculated using the longitudinally averaged remote pacific .
  ...
  \subsection{Accounting for smearing}
    ...
    \subsubsection{NOx dependence}
      ...
      At low NO (less than 50~pptv), reaction with HO$_2$, other RO$_2$, and isomerisation dominate the fate of RO$_2$.
  ...
  \subsection{Running GEOS-Chem with scaled isoprene emissions}
...
\section{Results}
  ...
  \subsection{HCHO Products and yield}
    ...
    First generation HCHO yield refers to the amount of HCHO produced per unit isoprene consumed by initial oxidation, total yield (or molar yield) refers to time dependent yield of HCHO over multiple oxidation stages \parencite{Wolfe2016}.
    ...
    In this work yield refers to total HCHO yield from isoprene oxidation.
    
    Modelled daily averaged yields can be qualitatively derived from the $slope$ using the daily averaged modelled loss rates ($L$) in $k_{hcho} = \frac{L}{[HCHO]}$ since $slope = \frac{Y_{isop}}{k_{HCHO}}$ from equation \ref{BioIsop:Method:slope:eqn_isop_to_hcho}.
    Figures TODO: and TODO show the daily averaged lifetime ($\tau = \frac{1}{k_{HCHO}}$) and yield ($Y_{isop}$) modelled by GEOS-Chem.
    Model output of loss rates are only available on a daily time scale, which limits us to an overview of modelled seasonality in this metric.
    ...
    \begin{table} \begin{threeparttable}
      ...
      \begin{tablenotes} % \item makes new lines
        ...
        \item g Australian average over austral summer from GEOS-Chem assuming 2.5 hour HCHO lifetime in this work.
      \end{tablenotes}
  ...
  \subsection{Emissions comparisons}
    ...
    Figure \ref{BioIsop:Results:Emissions:fig_time_series_vs_megan} shows emissions over Australia calculated using the OMI top down estimate (column 1: E$_{new}$) and MEGAN (column 2: E$_{MEGAN}$).
    The first row shows the overall average emission rates, the second row shows the time series (daily midday averages) and the final row shows the relative differences as $\frac{E_{MEGAN} - E_{new}}{E_{MEGAN}}$.
    ...
    \mypic{Figures/OMI_link/Emiss/E_zones_PP_lr.png}{%
      Emissions of isoprene estimates using OMI top down inversion (column 1: E$_{new}$) and MEGAN (column 2: E$_{MEGAN}$).
    ...
    \mypic{Figures/OMI_link/Emiss/E_zones_PP_monthly_lr.png}{ As figure \ref{BioIsop:Results:Emissions:fig_time_series_vs_megan} using monthly medians of the daily midday emissions estimates.}{\label{BioIsop:Results:Emissions:fig_time_series_vs_megan_monthly}}
...
\label{BioIsop:Uncertainty}
  ...
  Uncertainty in satellite HCHO, along with top down emissions estimates $E_{isop}$ is listed in table \ref{BioIsop:Uncertainty:tab_uncertainties}.
  The final determination of top-down emissions comes from equation \ref{BioIsop:Method:eqn_Enew}: $E_{isop}=\frac{VCC - VCC_{0}}{S}$.
  ...
  \begin{align*}
    ...
    VCC - VCC_{0} & = \Phi \\
    \Delta{\Phi} & = \sqrt{(\Delta{VCC})^2 + (\Delta{VCC_{0}})^2} \\
  ...
  In order to quantify uncertainty in $\Delta{E_{new}}$ we need the uncertainties in the underlying terms: $\Delta{S}$, $\Delta{VCC}$, and $\Delta{VCC_0}$. 
  For $\Delta{S}$ ($\Omega_{HCHO} = S \times E_{isop} + \Omega_0$ from equation \ref{BioIsop:Method:slope:eqn_isop_to_hcho}) we examine related GEOS-Chem output in section \ref{BioIsop:Uncertainty:Model}, since $S$ comes directly from the monthly linear regression of modelled isoprene emissions and column HCHO.
  Uncertainty in terms $VCC$, and $VCC_0$ come from both the OMHCHO slant columns ($SC$) and the AMFs calculated to transform them into vertical columns ($VC = SC/AMF$ from equation \ref{Model:AMF:eqn_AMFFrac}).
  Section \ref{BioIsop:Uncertainty:VCC} describes these calculations.
  
  The summation of these uncertainties through standard quadrature rules provides an estimate of random error in the calculation of $E_{new}$.

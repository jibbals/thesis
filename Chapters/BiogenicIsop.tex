...
\section{Outline TODO: move into intro?}
  ...
  %TODO Method outline to Jenny
  \begin{enumerate}
    \item Download Aqua/Terra MODIS gridded fire counts (MOD14A1), and Aura HCHO columns (OMHCHO).
    ...
    \item Mask HCHO columns which occur on days with non-zero (MODIS) fire counts over the prior 8 days, as is done in \cite{Marais2012} (I do not yet mask smoke aaod to account for transported plumes!).
    \item Filter gridsquares where OMI aaod is over some threshhold 
    \item yield stuff
    \item isoprene stuff

...
\label{BioIsop:intro}  
  ...
  \subsection{Existing emissions estimates}
    ...
    \added[id=tb]{This wide range demonstrates the effect of unknowns and uncertainties involved in isoprene emission estimation. Their results are compared to results here in Section} \ref{BioIsop:results:emissions}.
...
\section{Methods}
  ...
  \deleted[id=tb]{The following subsection (\ref{Model:filter}) has been moved here from Chapter \ref{Model}. I have added this to-be-removed paragraph instead of highlighting everything.}
...
\section{Results}
  ...
  \subsection{A posteriori emissions}
    ...
    They use a fully adjoint version of the IMAGESv2 CTM with emissions determined by MEGAN-MOHYCAN, and iteratively update biogenic isoprene emissions and pyrogenic VOC emissions so that the CTM produces HCHO columns that match (as near as possible) the column HCHO observed from OMI.
    The differences in results between their work and work performed here are most likely to be primarily driven by differences in modelled chemistry, rather than differences in top-down methodology.
    Both techniques have the end result of reducing the differences between (respective) modelled and OMI total column HCHO.

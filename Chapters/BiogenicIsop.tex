...
\section{Methods}
  ...
  \mypic{/Figures/OMI_link/GC/E_isop_vs_hcho_200501.png}{%
    Top left: RMA slope between modelled tropospheric column HCHO ($\Omega_{GC}$) and isoprene emissions ($E_{GC}$) using midday (13:00-14:00~LT) values over for January 2005, per gridsquare at \lowhr horizontal resolution.
  ...
  \subsection{Relationship between isoprene emissions and formaldehyde}
    ...
    This is derived using several assumptions that are important to understand.
    ...
    The third assumption is that HCHO is produced quickly from short-lived intermediates:
    \begin{eqnarray*}
    ...
    %Then assuming a steady state of atmospheric HCHO (total column HCHO: $\Omega$) produced by oxidation of VOC$_i$ without horizontal transport:
    ...
    Lumping non-isoprene precursors, and assuming that enhancements above the background are only due to isoprene emissions we have: 
    \begin{eqnarray*}
    k_{HCHO} \Omega_{HCHO} = & Y_{isop} E_{OMI} + \sum_{i \ne isop} Y_{i} VOC_{i} \\
    ...
    We look at each $2\times2.5$~\degr grid box from daily GEOS-Chem (biogenic only) output of $\Omega_{HCHO} \equiv \Ogc$ and $E_{OMI} \equiv E_{GC}$ within Australia, and calculate the (RMA) slope for each month.
  ...
  \subsection{Accounting for smearing}
    ...
    %%TODO: I was going to suggest some re-writes to this paragraph, but I think you can ditch it altogether and go straight to the next section where you explain this all much better and in detail
    ...
    %One would expect HCHO enhancement (above background levels) to be halved in isoprene emitting gridboxes if no transport has occurred.
    ...
    \subsubsection{NOx dependence}
      ...
      \mypic{Figures/OMI_link/Filters/smearing_nox_200501.png}{%
        Top left: OMNO2d tropospheric NO$_2$ columns (NO$_2$: molec cm$^{-2}$) averaged into \lowhr horizontal bins for over Jan, 2005.

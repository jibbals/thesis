...
\section{Methods}
  ...
  \label{BioIsop:method:scaled}
    ...
    After creating $E_{isop}$, the top-down estimate of isoprene emissions, we run GEOS-Chem again with biogenic emissions scaled to match the new estimate. 
    First all the new midday (13:00-14:00~LT) emissions (per grid box) and forming a multi-year monthly mean, which can be compared to the GEOS-Chem equivalent.
    ...
    Then the midday emissions for each month per gridbox are averaged and the multi-year average of these is compared against the top-down estimate.

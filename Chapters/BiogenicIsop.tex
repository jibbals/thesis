...
\section{Methods}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \item 
        Satellite measured HCHO columns (Section \ref{Model:omhcho}) are used to create a top-down estimate of biogenic isoprene emissions ($E_{post}$~atoms C cm$^{-2}$ s$^{-1}$).
      ...
      \item 
        A postiori top-down emissions $E_{post}$ are compared against a priori emissions, as well as analysed in conjunction with concentration measurement campaigns (MUMBA, and SPS).%, and one set of airplane measurements (HIPPO)).
  ...
  \subsection{Calculation of Emissions}
    ...
    These emissions are referred to as the a postiori from here on, or $E_{post}$ in formulae.
    ...
    \begin{eqnarray} \label{BioIsop:method:eqn_Enew}
      \Omega = & S \times E_{post} + \Omega_0 \notag \\
      E_{post} = & \frac{\Omega - \Omega{0}}{S}
    \end{eqnarray}
    ...
    This leaves $E_{post}$ as the only unknown once the satellite measurements are processed to match the temporal and horizontal resolution of $S$.
    ...
    When calculating the $E_{post}$ from our modelled slope, negative emissions result wherever measured columns are lower than background amounts (as $E_{post} = \frac{\Omega - \Omega_0}{S}$).
    These are set to zero, which increases monthly Australian $E_{post}$ averages by TODO: $\sim X-Y$~\% with the highest increases occurring during (TODO winter?).
    
    Top-down emission rates calculated in this work are in units of molecules cm$^{-2}$ s$^{-1}$.
    In order to calculate the emissions in kg, each grid square is multiplied by the area, and then daily emissions are assumed to follow a sine wave peaking at $E_{post}$.
  ...
  \label{BioIsop:method:scaled}
    ...
    The scaling factor $\alpha$ is the ratio between the multi-year averaged monthly emissions from GEOS-Chem $E_{GC}$ and the a postiori $E_{post}$: 
    \begin{equation} \label{BioIsop:method:scaled:eqn_alpha}
        \alpha = \frac{E_{post}}{E_{GC}}
    \end{equation}
    ...
    Figure \ref{BioIsop:method:scaled:fig_alpha_mya_series} shows $\alpha$ for the average January and June over 2005-2012, along with a time series of $E_{GC}$ and $E_{post}$ and $\alpha$ calculated for Sydney, and their multi-year seasonal average.
    ...
    \mypic{Figures/OMI_link/new_emiss/alpha_mya.png}{%
      ...
      Row 2: $E_{GC}$ (xxx, left axis), $E_{post}$ (xxx, left axis), and  $\alpha$(black, right axis) multi-year monthly averages calculated for Sydney.
...
\section{Results}
  ...
  \subsection{Literature comparisons}
    ...
    Figure \ref{BioIsop:results:emissions:fig_tga_comparison_map} shows how this decrease is distributed spatially, with $E_{GC}$ and $E_{post}$ in \tgpyr calculated as a multi-year mean.
    ...
    \mypic{Figures/OMI_link/Emiss/tga_map.png}{%
      Top row: multi-year mean emissions in \tgpyr from $E_{GC}$ (GEOS-Chem; running MEGAN) and $E_{post}$ (top-down emissions) respectively.
      $E_{post}$ uses an assumed sinusoidal daily cycle, with daylight hours perscribed for each month: see Section \ref{BioIsop:method:calculation}).
...
\label{BioIsop:uncertainty}
  ...
  \subsection{Top down emissions}
    ...
    Uncertainty in satellite HCHO $\Delta{\Omega}$, along with top down a postiori emissions estimates $\Delta{E_{post}}$ from literature is listed in Table \ref{BioIsop:uncertainty:eomi:tab_lit_uncertainties}.
    ...
    % UP TO HERE JENNY
    The summation of these uncertainties through standard quadrature rules provides an estimate of random error in the calculation of $E_{post}$.
  ...
  \subsection{Satellite Uncertainty}
    ...
    \subsubsection{OMI Retrieval}
      ...
      If we infer from this that there is 40\% bias and random uncertainty remains unchanged, then in the S term, we find TODO increase or decrease in $\Delta{E_{post}}$ of some amount through changing $\Delta{S}=$ .

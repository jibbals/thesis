...
\section{Results}
  ...
  \label{BioIsop:results:new_emiss}
    ...
    \subsubsection{Implications for HCHO}
      ...
      % TODO UPDATE TO RELATIVE DIFFERENCE IN THIRD COLUMNS
      \mypic{Figures/OMI_link/new_emiss/HCHO_total_columns_seasonal.png}{%
        Total column HCHO before (left) and after (middle) scaling isoprene emissions, and their relative differences (right). (TODO: Change absolute to relative differences, and fix colour bars)
      ...
      % Figure from ????
      % TODO: UPDATE ORDER TO OBS,PRIOR,NEW
    ...
    \subsubsection{Implications for ozone}
      ...
      % TODO: Add second y axis for absolute difference (+-3ppbv scale)
      % TODO: Update suptitle (O3 surface, not tropospheric column)
      ...
      %JENNY NOTE: some ordering issues: reads as if O3 hasn't decreased because of area averages! also could note that future work will downscale your emissions to evaluate impacts on O3 when spatial segregation of NOx and isoprene are properly accounted for. See Yu et al. ACP 2016 about resolution dependencies
      ...
      \mypic{Figures/OMI_link/new_emiss/O3_surf_map_seasonal.png}{%
        Surface (up to $\sim{150}$~m) ozone before (left) and after (middle) scaling isoprene emissions, and their relative differences (right). (TODO: UPDATE TO REL DIFF, UPDATE COLOUR BARS)
        Top row shows summer (DJF) averaged total columns, while bottom row shows the winter (JJA).
...
%      }{\label{BioIsop:results:new_emiss:trends:fig_no2_anomalies}}
  ...
  \subsection{Comparison with in situ measurements}
    ...
    \mypic{Figures/OMI_link/GC_VS_CAMPAIGNS_midday.png}{%
      ...
      Shown in magenta and cyan respectively are the a priori and a posteriori GEOS-Chem surface outputs for the matching grid square at midday for days containing measurements.
...
%    
  ...
  \subsection{Top down emissions}
    ...
    % Figure from Where??
    % TODO: shift x axis 0.5 to the right
...
\section{Conclusions and implications}
  ...
  % JENNY NOTE: What did Emmerson2019 do? could future work be to apply her results here?

...
\label{ch1:sec:emissions}
  ...
  \subsection{Satellite Measurements}
    ...
    This can be overcome if we know the sources of uncertainty and ensure they are not introducing systematic biases.
    By averaging over several measurements any random error can be reduced by a factor of one over the square root of the number of measurements.
...
\label{ch1:sec:HCHO}
  ...
  \subsection{Relationship with glyoxyl and isoprene}
    ...
    This process is similar to the proposed mechanisms for hydroperoxyaldehydes by (todo read abstract and cite peeters 2014 (\verb|http://dx.doi.org/10.1021/jp5033146|)) and carbonyl nitrates (todo: read and cite M\:{u}ller 2014).
    
  \subsection{Satellite measurements}
    Satellite measurements of HCHO are relatively uncertain, however this can be improved by averaging over larger gridboxes or longer time scales.
...
\label{ch1:sec:isoprene}
  ...
  \subsection{Isoprene From HCHO}
    ...
    Estimates of the HCHO yield (Y$_i$) can be attained from a model as shown in \citet{Millet_2006}.

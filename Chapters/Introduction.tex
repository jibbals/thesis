...
\label{LR:Atmos}
  ...
  \label{LR:Atmos:Chem}
    ...
    Hydroxyl \replaced[id=tb]{(OH$\dot{}$) and hydroperoxyl (HO$_2$) radical}{radicals (OH$\dot{}$) and hydrogen dioxide (HO$_2$)} concentrations largely determine the oxidative capacity of the atmosphere.
    ...
    Since radicals are involved in all oxidative chemistry in the atmosphere it is important for models to accurately represent them. 
...
\label{LR:O3}
  ...
  \subsection{Stratospheric ozone}
    ...
    The lifetime of O against loss by O$_2$ is less than a second in the troposphere, and produced O$_3$ quickly returns to O and O$_2$, as \replaced[id=tb]{lower energy ($\lambda < 350$~nm) photons are abundant}{low energy ($\lambda < 1180$~nm) photons and M are abundant}.
    The reduced light penetration towards the surface, in addition to the logarithmic increase in atmospheric pressure (which affects M abundance) \replaced[id=tb]{results in a vertical profile of ozone that peaks in the stratosphere.}{drives the vertical profile of ozone into what is called the \textit{ozone layer}.}
    This is a layer of relative ozone abundance within the stratosphere \added[id=tb]{called the \textit{ozone layer}}.
  ...
  \subsubsection{Stratosphere to troposphere transport}
    ...
    The proportion was revised down to around 10\% over the years as measurement and modelling campaigns improved\deleted[id=mh]{ our} understanding of global scale transport, mixing, and chemistry \parencite{Guenther2006, Monks2015}.
  ...
  \subsubsection{Chemical production}
    ...
    % With the NO$_3$ reactions occurring within seconds.
    %%NO_2 + O_3       & \to NO_3 + O_2      && \\%
...
\label{LR:VOCs}
  ...
  \added[id=tb]{These properties are largely dictated by the chemical makeup and structure of the individual compounds. A compound's atmospheric lifetime is strongly related to its reactivity (and the concentration of reactants), with more reactive compounds having shorter atmospheric lifetimes.}
  ...
  \added[id=tb]{Globally, biogenic sources dominate emissions of VOCs \parencite{Guenther1995, Kanakidou2005, Millet2008, Guenther2012}, although anthropogenic and pyrogenic sources can also be important in specific environments \parencite[e.g.,][]{Zhu2014a, Kwon2017}.}
  \deleted[id=tb]{Understanding the drivers of trends in biogenic VOC emissions (BVOCs) is required in order to estimate future carbon fluxes, changes in the water cycle, ozone production, air quality, and other climate responses \parencite{Yue2015}.}
  ...
  \added[id=tb]{Understanding the drivers of trends in biogenic VOC emissions (BVOCs) is required in order to estimate future carbon fluxes, changes in the water cycle, ozone production, air quality, and other climate responses \parencite{Yue2015}.}
  ...
  \subsection{Emissions}
    ...
    \added[id=tb]{This thesis mostly focuses on biogenic emissions, with influences from pyrogenic and anthropogenic emissions removed (Section \ref{Model:filter}) and not discussed further.}
    ...
    \replaced[id=mh]{This thesis focuses }{In this thesis I focus }on emissions of isoprene in Australia.
    ...
    \deleted[id=tb]{Many of these estimates come from Model of Emissions of Gases and Aerosols from Nature (MEGAN), a bottom-up biogenic emissions model.}
    \deleted[id=tb]{MEGAN parameterises BVOC emissions as a function of temperature, land cover, plant functional type, etc., with descriptions given in Guenther et al. (2012).}
    
  
  \subsection{Biogenic emissions modelling}
    \label{LR:VOCs:models}
    
    %% Background to biogenic modelling?
    %% bottom up approach
    
    \added[id=tb]{Many of the global estimates given in the paragraph above come from Model of Emissions of Gases and Aerosols from Nature (MEGAN), a bottom-up biogenic emissions model.
    Biogenic emissions models generally take bottom-up approach, where the environment is simulated and then the emissions from that environment are estimated based vegetation type, climate, and other local environmental factors. 
    The most important parameters dealing with VOC emissions are leaf area index, emission factors, plant functional type, light availability, and temperature \parencite{Messina2016}.
    The estimated emission factors are built up from measured plant emissions, which can be measured downwind \parencite[e.g.,][]{Nguyen2014} or from within a chamber \parencite[e.g.,][]{Paulot2009b,Nguyen2014}.
    There are several models that estimate VOC emissions, such as MEGAN (created and published by \parencite{Guenther1995}, with subsequent updates in \textcite{Guenther2000, Guenther2006, Guenther2012}), LPJ-GUESS (Lund-Potsdam-Jena General Ecosystem Simulator) \parencite{Arneth2007}, and ORCHIDEE (Organising Carbon and Hydrology in Dynamic EcosystEm) \parencite{Messina2016}. 
    Many of the available bottom-up models have inputs and sensitivities similar to those of MEGAN.
    MEGAN is used within this thesis as a module within a chemical transport model (Section \ref{Model:GC:MEGAN}).}
    
    

    
    %% HOW estimates are made    
    \added[id=tb]{The general formula governing modelled emissions $E$ for a species $i$ (from \textcite{BrasseurJacob2017}) is as follows:}
    \begin{equation}
      E_i = A \times EF_i \times S_i
    \end{equation}
    \added[id=tb]{with $A$ the activity rate, $EF_i$ being the emission factors, and $S_i$ is a scaling factor accounting for meteorology and other effects not included in $A$ or $F$ (e.g., seasonal temperature).
    For example, to estimate isoprene emission $E_{isop}$ per second for an area: emissions are $A=$ how many trees in an area multiplied by $EF_{isop}=$ isoprene emitted per tree per second multiplied by $S_{isop}=$ a scaling factor based on temperature and soil moisture.
    This is a simplified example of a bottom-up estimate for isoprene emissions.}
    ...
    %% Limitations
    \added[id=tb]{There is a potential issue caused by shared underlying algorithms and assumptions between different emission models, which lead to an unrealistic convergence on isoprene emission estimation \parencite{Arneth2008}.
    MEGAN can depend greatly on parameterised inputs such as temperature, sunlight, plant type, plant coverage, plant health, etc. \parencite{Arneth2008, Niinemets2010}.
    Sensitivity to these factors is pervasive in bottom up emissions models \parencite[e.g.,][]{Marais2014, Miller2014, Messina2016}.
    %The global emission factors used to derive these estimates are based on modelling emissions from different plant species (phenotypes), and 
    Relatively few Australian species are used within MEGAN's plant functional type parameterisations, and those that are may be poorly represented \parencite{Emmerson2016}.
    Additionally there is uncertainty around the soil moisture parameterisations, with soil moisture and its impacts on BVOC emission poorly understood in Australia \parencite{Sindelarova2014, Emmerson2019}.
    ...
    \added[id=tb]{The other common emission estimation technique is termed top-down.
    Top-down emissions estimates are derived from measurements of the child products of emitted VOCs.
    %For example, GOME satellite HCHO and a Bayesian inversion technique have been used to estimate isoprene emissions of $\sim566$\tgcpyr ~globally \parencite{Shim2005}.
    Based on top-down emissions estimates, isoprene emissions appear to be overestimated in Australia.
  ...
  \label{LR:VOCs:Isop}
    ...
    MEGAN \replaced[id=tb]{ and other models}{(Guenther et al. 1995), and subsequent updates (Guenther et al. 2000; Guenther et al. 2006; Guenther et al. 2012),} have been used throughout the atmospheric community to make global estimates of isoprene emissions, at roughly 500-600\tgpyr, emitted mostly during the day.
    \deleted[id=tb]{Australia has increased uncertainty (relative to globally) due to its lack of measurements and study  (see Section \ref{LR:VOCs:models}).}
    \deleted[id=tb]{Other models also exist which estimate similar isoprene amounts, such as LPJ-GUESS (Lund-Potsdam-Jena General Ecosystem Simulator) (Arneth et al. 2007), and ORCHIDEE (Organising Carbon and Hydrology in Dynamic EcosystEm) (Messina et al. 2016).
    There is a potential issue caused by shared underlying algorithms and assumptions between the models, which lead to an unrealistic convergence on isoprene emission estimation (Arneth et al. 2008).
    ...
    %\parencite{Messina2016}.
    The global emission factors used to derive these estimates are based on modelling emissions from different plant species (phenotypes), and relatively few Australian species were used when forming these estimates.
    This leads to increased uncertainty for Australian emissions.}
  ...
  \subsection{Isoprene chemistry}
    ...
    \mypic{Figures/Mao2013_isop_hcho.png}{%
      Simplified isoprene products following oxidation by OH, reproduced from \textcite{Mao2013}.
    ...
    \subsubsection{Low NO$_x$ pathway}
      ...
      %These reactions are highly non-linear and subject to large uncertainty, however they were shown to improve modelled HO$_x$ concentrations against several campaigns.
...
\label{LR:HCHO}
  ...
  \subsection{Measurement techniques}
    ...
    % examine a single air mass with 8 instruments using the four techniques (MAX-DOAS, FTIR, chromatographic, and fluorimetric), and show that reasonable agreements can be achieved.
    ...
    \subsubsection{Satellite measurements}
      ...
      %such as from the Studies of Emissions and Atmospheric Composition, Clouds and Climate Coupling by Regional Surveys HCHO measurements over the southeastern US 
    ...
    \subsubsection{DOAS}
      ...
      In the atmosphere\replaced[id=mh]{, the optical depth of various chemical species can be determined using incoming solar radiation.}{we are interested in the optical depth of various chemical species, and we use incoming solar radiation to determine this.}
...
\label{LR:Models}
  ...
  Models can fill the gaps (both spatial and temporal) in measurement records, and\deleted[id=mh]{ can help us} improve\deleted[id=mh]{ our} understanding of the natural world.
  For example, they can be used to examine future outcomes resulting from changing\deleted[id=mh]{ our} emissions.
  They can be used to increase measurement accuracy (for instance in satellite measurements) and determine where \replaced[id=mh]{information is lacking}{we lack information}, while also evaluating the performance of new instruments.
  \added[id=tb]{Some advanced measurement techniques make use of modelled a priori information in order to produce useful outputs.
  ...
  \subsection{Chemical transport models}
    ...
    \added[id=tb]{A chemical mechanism is a closed system of chemical reactions and their associated rate constants.
    ...
    \added[id=tb]{The major chemical transport model used in this thesis is GEOS-Chem, an Eulerian 3-D box model as discussed above.
    ...
    \deleted[id=tb]{Section 1.5.3 Emissions has been moved from here into new Section 1.3.2}
    
    \deleted[id=tb]{The general formula governing modelled emissions $E$ for a species $i$ (from \textcite{BrasseurJacob2017}) is as follows ...}
  ...
  \label{LR:Models:Uncert}
    ...
    \subsubsection{Emissions Inventories}
      ...
      Bottom up inventories of BVOCs \added[id=tb]{(Section \ref{LR:VOCs:models})} remain largely uncertain due to missing or extrapolated plant functional type information, changing land cover, and parameterised environmental stressors \parencite{Guenther2000, Kanakidou2005, Millet2006}.
      ...
      Estimates of isoprene emission are based on a few algorithms that \replaced[id=tb]{may be heavily influenced by parameterised inputs (see Section \ref{LR:VOCs:models}).}{can depend greatly on parameterised inputs such as temperature, sunlight, plant type, plant coverage, plant health, etc. (Arneth et al. 2008; Niinemets et al. 2010).}
      Sensitivity to these factors is pervasive in bottom up emissions models \parencite[e.g.,][]{Marais2014, Miller2014, Messina2016}.
      \textcite{Arneth2008} argue that the monopoly of isoprene emission estimates may be leading\deleted[id=mh]{ us} to an incorrect understanding of isoprene chemistry.
      ...
      % soil moisture stuff
      \deleted[id=tb]{Modelled emissions are sensitive to soil moisture, especially near the wilting point, below which trees stop emitting isoprene and other VOCs completely as they can no longer draw water (Bauwens et al. 2016).
      ...
      \added[id=tb]{For example,} \textcite{Jiang2018} found that improving \replaced[id=tb]{MEGAN}{the} parameterisation of drought based on a measurement campaign in the U.S. would lower isoprene emissions globally by $\sim17\%$.
    ...
    \subsubsection{Chemistry mechanisms}
      ...
      \added[id=tb]{Until recently, models struggled to represent OH in the atmosphere, due to missing chemistry and issues in measurement techniques.
      ...
      They showed that photolysis of photolabile peroxy-acid-aldehydes generates OH, improving model agreement with continental observations.
      ...
      \textcite{Mao2013} showed that drastically lowering (by a factor of 50) the rate constant for \roo isomerisation led to better organic nitrate agreements with measurements.% (ICARTT).
...
\label{LR:Aus}
  ...
  \subsection{Ozone}
    ...
    This affects\replaced[id=mh]{ the scientific community's}{ our} ability to accurately determine sources of ozone in the troposphere, with current southern hemisphere trends lacking full explanation \parencite{Zeng2017}. 
    ...
    Quantifying ozone processes over the southern ocean could improve\deleted[id=mh]{ our} understanding of chemistry in the ``clean background environment'', while additionally helping to validate model and satellite datasets.
  ...
  \subsection{Biogenic VOCs}
    ...
    Figure \ref{LR:Aus:fig_forests} shows the different forest types and their locations within Australia, highlighting that much of\replaced[id=mh]{ the}{ our} forested lands are frequently found near population centres along the east coast.
    ...
    It has been estimated by\replaced[id=tb]{ the MEGAN biogenic emission model (Section \ref{LR:VOCs:models})}{ MEGAN} that Australian vegetation is among the world's strongest isoprene emitters, with forests in SE Australia having emission factors greater than 16 mg m$^{-2}$ h$^{-1}$ (see figure \ref{LR:Aus:fig_MEGAN_EF}) \parencite{Guenther2006, Guenther2012}.
...
\section{Summary of gaps in knowledge}
\label{LR:Gaps}
  
  \added[id=mh]{%
    Several layers of knowledge are required to accurately estimate ozone concentration in the troposphere, with the most important being emissions of precursors, transport from the ozone rich stratosphere, and chemical processes in both clean and polluted air masses.
    These natural processes that lead to ozone production are poorly understood in Australia mostly due to the difficulty and expense involved in measuring both ozone and precursors over any representative sample of Australia.
    Improvements to emissions models require improved understanding of emissions from Australian vegetation and how they respond to meteorological and chemical parameters.
    How Australian tropospheric ozone concentrations are affected by ozone transported from the stratosphere is also unclear, largely due to a lack of knowledge around how frequently a transport event occurs, and the quantity of ozone transported by any such event.
    Finally, due to the complex nature of atmospheric chemistry involving isoprene and other BVOCs, it is not currently known how sensitive ozone concentrations in the atmosphere are to changes in BVOC emissions.
    }
  
\section{Aims}
\label{LR:Aims}

  \textbf{\replaced[id=mh]{This thesis aims }{In this thesis I aim }to improve understanding of natural contributions to ozone over Australia and the southern ocean.}
  ...
  \replaced[id=mh]{This thesis aims }{I aim }to improve understanding of both of these sources using existing satellite and ground-based datasets along with modelled outputs from the GEOS-Chem chemical transport model.
  ...
  Combining satellite data with model outputs provides a platform for the understanding of natural processes\deleted[id=mh]{, which are uncertain over Australia}.
  Satellite measurements require modelled a priori vertical profiles of HCHO to estimate total column amounts.
  \textbf{\replaced[id=mh]{This thesis aims }{I aim }to recalculate satellite vertical columns of HCHO using updated model a priori information.}
  In this effort \deleted[id=mh]{I aim to improve the understanding of }the importance of relevant parameters (within GEOS-Chem) in calculating vertical columns of HCHO measured by satellite \added[id=mh]{is analysed}.
  ...
  Additionally, \deleted[id=mh]{I detail }the construction and effects of satellite data filters \added[id=mh]{is examined}.
  ...
  \textbf{\replaced[id=mh]{This thesis aims }{I aim }to determine isoprene emissions in Australia using a top-down inversion of satellite HCHO, through a modelled yield from isoprene to HCHO}.
  ...
  \textbf{\replaced[id=mh]{This thesis aims }{I aim }to improve understanding of ozone transported to the troposphere from the stratosphere in Australia and the southern ocean.}
  Ozonesondes provide a glimpse of the vertical ozone profile up to $\sim 30$~km\replaced[id=mh]{. In this thesis }{, and I use }a Fourier filter \added[id=mh]{is used} to determine how often stratospheric transport is occurring at three sites: Melbourne, Macquarie Island, and Davis Station. 
  ...
  \textbf{Overall, \replaced[id=mh]{this thesis aims }{I aim }to describe relative importance of sources of tropospheric ozone in Australia, as well as their seasonality.}
  \replaced[id=mh]{This includes a description of }{I will describe }how modelled ozone is affected by updated isoprene emissions, \replaced[id=mh]{a comparison of }{comparing }changes in GEOS-Chem outputs, and \replaced[id=mh]{an analysis of }{by }ozone transport from the stratosphere.\deleted[id=mh]{.}

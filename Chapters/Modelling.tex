...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Creating new shape factors}
    ...
    The shape factor (\textit{S}) is one of the key components in creation of the AMF (see section \ref{Model:Meas:sat}).
    ...
    The AMF is determined using scattering weights ($\omega$) and shape factors based on a priori estimates of HCHO vertical profiles.
    ...
    % Calculations from GEOS-Chem output which lead to shape factor
    TODO: GEOS-Chem output -> Shape factor per grid square
  ...
  \subsection{Creating the new AMF}
    ...
    \subsubsection{vertical shape factor}

...
\section{GEOS-Chem}
  ...
  \subsection{GEOS-Chem simulations}
    ...
    \subsubsection{UCX vs tropchem}
      ...
      Total column HCHO ($\Omega_{HCHO}$) does not differ much between runs.
      The differences do not exceed 3\% over Australia for the averaged month of January, 2005.
      The major notable difference occurs over northern Africa, where $\Omega_{HCHO}$ is around 20\% lower in the UCX run.
      Otherwise the only difference is a slight ($<8\%$) decrease in $\Omega_{HCHO}$ over the oceans.
      The comparison is repeated using total columns (instead of surface values) in figure TODO, showing that differences affecting HCHO between the model run are spread over the entire vertical column.
      
      
      \begin{figure} % TODO: remove 'rerun' plot, use normal and delete rerun thingy so it's not confusing later.
      ...
      Very little isoprene is seen away from the continent (4-5 orders of magnitude less), due to the short lifetime of isoprene, and a lack of sources over the oceans.

...
\section{Recalculation of OMI HCHO}
  ...
  When comparing OMI VCs to GEOS-Chem, the satellite AMF needs to be recalculated using GOES-Chem modelled vertical gas profiles as the aprioris.
  ...
  % Very brief outline
  In recalculating AMFs, the shape factors used by OMI are replaced using GEOS-Chem profiles.
  Additionally both the shape factor and scattering weights of the satellite are recalculated using code initially written by Professor P. Palmer (see sections \ref{Model:omiRecalc:ShapeFactor} and \ref{Model:omiRecalc:ppcode} for more details).
  Finally a reference sector correction is determined using a similar method to that seen in \textcite{Abad2016}, (see section \ref{Model:omiRecalc:RSC}).
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \item For each pixel, a new AMF is created using the GEOS-Chem shape factors and satellite scattering weights.
      \begin{enumerate}
        ...
        \end{align}
      \end{enumerate}
      \item The AMFs are used to determine the new vertical columns: $\Omega_{HCHO} = SC/AMF$.
      \item Another recalculation (AMF$_{PP}$) using code from P. Palmer and L. Surl which additionally recalculates the $\omega$ for each pixel is described in section \ref{Model:omiRecalc:ppcode}.
      ...
      \end{enumerate}
      \item Corrected vertical columns (VCC) are determined for each pixel using $VCC = (SC - RSC(lat,track) )/AMF$ for each AMF.
      \item The VCC and $\Omega_{HCHO}$ are binned onto a 0.25\degr by 0.3125\degr grid along with how many pixels have been binned, and the average pixel uncertainty in product OMHCHORP.
    \end{enumerate}

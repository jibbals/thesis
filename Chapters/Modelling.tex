...
\section{Filtering Data}
  ...
  \subsection{Anthropogenic filter}
    ...
    \mypic{Figures/OMI_link/OMNO2_thresheffect_2005_final.png}{%
  ...
  % JENNY TODO:
  %I think this section is a bit problematic. First, it doesn't really fit here -- I would suggest if you keep it (keep reading) it fits better as a subsection to the pyrogenic filter, because the point is to test whether the pyrogenic filter is adequately filtering out fire influence. Second, it really only matters if it is working for the OMI data, not the GEOS-Chem output, right? Because we KNOW we turn off fires for GEOS-Chem... Third, there seem to be two different forms of the analysis and the flow in this section isn't great. And finally, I don't think the results are all that compelling at the end of it.
  
  % JENNY TODO continued
  %What I would suggest is that at the end of your pyrogenic filter section, you add a very brief paragraph to state that you attempted to test the pyrogenic filter by looking at the relationship between temperature and HCHO, which should be exponential if it is driven by isoprene emissions but with outliers possibly indicative of fire influence. You then say something non-committal about this analysis not proving very useful because of <the reasons at the start of your last paragraph here where you talk about problems>, although there was a moderate improvement in the correlation between HCHO and temperature when the filter was applied. You then refer the reader to Appendix A, and basically drop this whole section (including figures) into an appendix at the end. Sound good?

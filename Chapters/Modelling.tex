...
\section{GEOS-Chem}
  ...
  \subsection{GEOS-Chem outputs}
    ...
    \begin{description}
      ...
      \item[Tracer averages]%
        ...
        These are used for several purposes including ozone intrusion quantification (see section \ref{Ozone:fig:GEOSChemSeasonalProfiles}), and isoprene emissions estimation sensitivities (see section \ref{BioIsop:Method:Smearing}).
      \item[Time series]%
        are 2x2.5\degr vertical columns it is possible to save diagnostics at a temporal resolution of up to 15 minutes.
        TODO: How am I using this output (Ozone comparison?)
  ...
  \subsection{GEOS-Chem simulations}
    ...
    Run descriptions and their outputs are listed here, with outputs described in more detail in section \ref{Model:GC:outputs}:
    \begin{enumerate}
      \item UCX 
      \begin{enumerate}
        \item Satellite output (1300-1400~LT, daily)
        \item Determine ozone seasonality in the stratosphere and troposphere to quantify ozone flux influence.
        \item Check how shape factors for AMF recalculation are affected by vertical resolution in OMI
        TODO: Make a plot of AMF with without 72 vertical levels
      ...
      \begin{enumerate}
        \item Satellite output (1300-1400~LT, daily), daily tracer averages
        \item Recreate the AMFs for OMI
        \item Additional output to run code from Prof. Paul Palmer, modified by Dr. Luke Surl.
      ...
      \begin{enumerate}
        \item Satellite output (1300-1400~LT, daily)
        \item Monthly averaged tracer concentrations
        \item Check ozone sensitivity to isoprene emissions
        \item Determine model sensitivity transport (smearing)
        \item Smearing is determined by running the model with the biogenic isoprene emissions halved while other parameters remain unchanged (see \ref{BioIsop:Methods:Smearing}).
      \end{enumerate}

...
\section{Introduction}
  ...
  \added[id=tb]{A more detailed discussion of CTMs can be found in Section \ref{LR:Models:ctm}.}
  \deleted[id=tb]{Some advanced measurement techniques make use of modelled a priori information in order to produce useful outputs.
  Models of ozone in the atmosphere are used broadly for international assessments of ozone precursor emissions, and estimating effects from related processes (such as radiation) (Young et al. 2017).
  ...
  %Many data points can be averaged in order to reduce uncertainty.
  In this chapter several satellite datasets are combined to estimate biogenic HCHO amounts over Australia.
  
  The first aim is to analyse Australia-specific HCHO amounts measured by satellite.
  ...
  \deleted[id=tb]{The creation and effects of filters used to remove non-biogenic influences are described in Section 2.7.}
...
\section{Datasets}
  ...
  This can be useful when running global atmospheric models as the ground altitude is always at $\sigma=1$ and \replaced[id=mh]{topography is not an issue}{we need not worry about topography}.
  ...
  \label{Model:datasets:satellite}
    ...
    %The use of multiple satellites can even be used to detect intradiel concentrations in trace gas columns, as shown in \textcite{Stavrakou2015} using OMI and GOME-2 measurements, which have respective overpass times of 1330 and 0930 local time.
    ...
    \subsubsection{Formaldehyde}
      ...
      \added[id=mh]{Satellite formaldehyde data are used in this thesis to constrain Australian isoprene emissions (Chapter \ref{BioIsop}).}
    ...
    \subsubsection{Nitrogen dioxide}
      ...
      \mypic{Figures/OMNO2d_2005m0129.png}{%
        ...
        Data downloaded from \url{https://search.earthdata.nasa.gov/search}, DOI:10.5067/Aura/OMI/DATA3007.
    ...
    \subsubsection{Aerosol optical depth}
      ...
      A data product provided by Earthdata (\url{https://disc.gsfc.nasa.gov/datasets/OMAERUVd_V003/summary}) called OMAERUVd (DOI: 10.5067/Aura/OMI/DATA3003) is used in this thesis \added[id=mh]{to filter out satellite HCHO data that may be influenced by pyrogenic emissions (see Section \ref{Model:filter:fire})}.
    ...
    \subsubsection{Active fires}
      ...
      \added[id=mh]{Filtering pyrogenic influence from satellite HCHO requires knowledge of where fire is occurring, which is determined using the MOD14A1 dataset as applied in Section \ref{Model:filter:fire}.}
      ...
      This product is downloaded and binned into the lower \highhr ~resolution (using the sum of fire pixels) to create an active fire influence mask\deleted[id=mh]{ (see Section \ref{Model:filter:fire})}.
  ...
  \subsection{Model datasets}
    ...
    \label{Model:datasets:model:CPC}
      ...
      \mypicw{.5\textwidth}{Figures/CPC_Temperature_EG.png}{%
        Climate Prediction Center daily maximum temperature data for 1 Jan. 2005.
  ...
  \subsection{In situ and ground based datasets}
    ...
    \label{Model:datasets:wollongong_ftir}
      ...
      % Picture from tests/campaign_tests.ftir_method_plots
...
\label{Model:GC}
  ...
  \subsection{Overview}
    ...
    In this thesis\replaced[id=mh]{ GEOS-Chem version 10.01 is used}{ I use version 10.01 of GEOS-Chem}, which outputs up to 66 chemical species (tracers) in the standard run, at \lowhr ~horizontal resolution, with 47 levels up to the top of the atmosphere (TOA at 0.01~hPa).
  ...
  \subsection{Installing and running GEOS-Chem}
    ...
    In order to build and run GEOS-Chem a high-speed computing system is optimal, as globally gridded chemical calculations can take a long time to perform (\deleted[id=mh]{for us }$\sim 70$ computation hours per month \added[id=mh]{in this work}).
    \replaced[id=mh]{For this thesis, GEOS-Chem is installed}{I installed GEOS-Chem} onto a suitably configured workspace on the Raijin supercomputer at the National Computational Infrastructure (NCI, \url{http://nci.org.au/}).
  ...
  \subsection{Chemical Mechanism}
    ...
    \deleted[id=tb]{A chemical mechanism is a closed system of chemical reactions and their associated rate constants.
  ...
  %\subsection{GEOS-Chem isoprene oxidation}
    \label{Model:GC:Isop}
    ...
    %They used anion chemical ionisation mass spectrometry (CIMS) to determine products of isoprene photooxidation.
    ...
    \deleted[id=tb]{In part, GEOS-Chem chemistry mechanisms are based on atmospheric chemistry chamber studies.
    ...
    Formation of isoprene nitrates (ISOPN) affect ozone levels through NO$_X$ sequestration, the yields and fate of these nitrates was analysed in Paulot et al. (2009a).
    %% ISOPN can be oxidised (by OH) to form nitrated organic products \parencite{Paulot2009a}.
    Prior to 2012, oxidation chamber studies were performed in high NO or HO$_2$ concentrations, giving peroxy lifetimes of less than 0.1~s (Crounse et al. 2012; Wolfe et al. 2012).
    ...
    \deleted[id=tb]{Crounse et al. (2011) examined the isomerisations associated with the oxidation of isoprene to six different isomers of \roo formed in the presence of oxygen.}
    ...
    \subsubsection{Nitrogen oxide impacts}
      ...
      \replaced{Isoprene reactions that occur following emission }{GEOS-Chem reactions do not distinguish between high and low NO$_x$ scenarios.
      ...
      \added{This leads to isoprene reactions being broadly discussed within the context of high or low NO$_x$ environmental conditions.}
      ...
      %low NO$_X$ concentrations are around 0.1~ppb.
      ...
      %Under low NO$_X$ conditions the expected production of HCHO is 4.7\%. %, MVK, and MACR is 7.3\%, and 12\% respectively.
      % This only occurs in one of the two loss mechanisms for isopoo, reaction with ho2
      ...
      % , which shows the production from \roo and NO.
    ...
    \subsubsection{OH regeneration}
      ...
      \deleted[id=tb]{Until recently, models struggled to represent OH in the atmosphere, due to missing chemistry and issues in measurement techniques.
      Prior to Mao et al. (2012), measurements of OH in high VOC regions may have been up to double the real atmospheric OH levels, due to formation of OH inside measuring instruments.}
      ...
      \deleted[id=tb]{Photolysis of photolabile peroxy-acid-aldehydes generates OH, improving model agreement with continental observations.
      OH and HPALD interactions are central to maintaining the OH levels in pristine and moderately polluted environments, which makes isoprene both a source and sink of OH Peeters and Muller 2010; Taraborrelli et al. 2012).
      Mao et al. (2013) showed that drastically lowering (by a factor of 50) the rate constant for \roo isomerisation lead to better organic nitrate agreements with measurements.% (ICARTT).
  ...
  \subsection{Biogenic emissions from MEGAN}
    ...
    %MEGAN can be run on its own as a global model,  with resolution of around 1~km, which uses globally estimated or measured %(including remote sensed satellite data) 
    ...
    In GEOS-Chem MEGAN uses high resolution emission factor maps described in \textcite{Guenther2012}. % instead of PFTs
    ...
    GEOS-Chem computes some emissions using predefined emission factor ($\epsilon$) maps from MEGAN source code, and others using PFT maps and associated emission factor relationships.
  ...
  \subsection{Nitrogen oxides}
    ...
    Other NO$_X$ emissions arise from sources including soil emissions %($\sim 10$~Tg N a$^{-1}$)
    and lightning.
    ...
    Simulated GEOS-Chem tropospheric NO$_2$ columns averaged from 13:00-14:00 local time are compared against OMNO2d data that are averaged into seasonal \lowhr ~bins. %(Sec. \ref{Model:datasets:OMNO2d}). 
    ...
    Additionally, high satellite NO$_2$ columns are filtered in later calculations that assume biogenic air masses as these high NO$_2$ columns suggest anthropogenic influence.
    ...
    \mypic{Figures/OMI_link/GC/NO_regressions_2005.png}{%
      The first column shows scatter plots of tropospheric NO$_2$ columns between GEOS-Chem (y-axis) and OMNO2d (x-axis) at \lowhr.
      ...
      The third column has a different x-axis scale for each season.
      These right two columns share the far right axis; however, emissions are from anthropogenic and soil sources respectively.
...
% Those figues used to go here... replaced by NO regressions figure
  ...
  \subsection{GEOS-Chem simulations}
    ...
    \subsubsection{UCX vs tropchem}
      ...
      Here\deleted[id=mh]{ we start to see} a higher relative difference in concentrations \added[id=mh]{becomes apparent}, although this is generally over the areas with lower absolute concentrations. 
      ...
      %  Midday (1300-1400~local time) surface OH concentrations averaged over Jan-Feb, 2007. 
...
\label{Model:omhcho}
  ...
  In this thesis\deleted[id=mh]{ I use} the NASA OMHCHOv003 data product \parencite{Abad2015} \added[id=mh]{is used}, with HCHO determined using the spectral window $328.5$~nm$ - 356.5$~nm. 
  ...
  \subsection{Uncertainty}
    ...
    The main sources of error in satellite retrievals of HCHO are instrument detection sensitivities and calculation of the AMF (the factor calculated to convert slanted light path concentrations into a vertical profile) \parencite{Millet2006}.
    ...
    If\replaced[id=mh]{ it is assumed that}{ we assume} the uncertainty is random error, and not bias introduced through calculation techniques, then \replaced[id=mh]{this uncertainty can be reduced}{we are able to reduce the uncertainty} through averaging.
    ...
    %A good example can be seen in \textcite{Zhu2014} where 0.2\degr ~by 0.2\degr ~resolution with high enough sensitivity to see anthropogenic HCHO is achieved with three summers worth of satellite data.
    ...
    \textcite{Zhu2016} examine OMHCHO and show a bias of 37\% %20 - 51\% instrument ranges
...
\section{Calculating new AMF}
  ...
  Using the log of Beer's law (Equation \ref{LR:HCHO:DOAS:eqn_beerslaw}) \replaced[id=mh]{gives}{we get} 
  ...
  \end{equation}
  %{LR:HCHO:DOAS:eqn_backscatterextinction}
  %$\tau_s$ is the optical thickness of the absorber along the measured path between source and instrument.
  
  \deleted[id=mh]{We use} $\nabla I = I_B - I_{B_0}$ \added[id=mh]{is used} to represent the change in intensity due to the absorber. 
  For optically thin absorption, $\nabla I / I_B << 1$\replaced[id=mh]{, which allows one to}{, and we can} use:
  ...
  \end{equation*}
  \replaced[id=mh]{Conversion of $\text{d}\tau$ to the satellite path is performed}{We can then convert $\text{d}\tau$ to our path} using Equation \ref{LR:HCHO:DOAS:eqn_tau} leading to
  ...
  \begin{align*}
    \alpha_e &= \frac{1}{\Omega_v} \int_0^\infty \alpha(z) \eta(z) \mathrm{d}z \\
     &= \frac{\tau_v}{\Omega_v}
  \end{align*}
  Then replacing the $\tau_v$ in Equation \ref{Model:AMF:eqn_AMFcross} \replaced[id=mh]{provides}{we obtain}:
  ...
  Plugging Equations \ref{Model:AMF:eqn_omega} and \ref{Model:AMF:eqn_shapefactor} into Equation \ref{Model:AMF:eqn_AMFpreomega} gives\deleted[id=mh]{ us}:
  \begin{equation} \label{Model:AMF:eqn_AMF_amfgintwSdz}
    AMF = AMF_G\int_0^\infty{ \omega(z) S(z) \mathrm{d}z}
  \end{equation}
  Since \replaced[id=mh]{this thesis uses}{we are using} the $\omega$ provided by OMI, the AMF$_G$ term is removed from this calculation as it is not part Equation \ref{Model:AMF:eqn_omega} leading to 
  ...
  In the sigma coordinate system\deleted[id=mh]{ we calculated} the shape factor as defined in \textcite{Palmer2001} \added[id=mh]{is calculated as follows}:
  \begin{equation}
  ...
  \replaced[id=mh]{Then one}{We} can integrate over the sigma coordinates using the hydrostatic relation $p = - \rho_a g z$, with $\rho_a$ being air density, and $g$ the gravitational acceleration:
  \begin{align*}
...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Outline}
    ...
    An outline in computational order of what takes place when recalculating $\Omega$ from OMI follows:
    \begin{enumerate}
    ...
    \added[id=mh]{Figure \ref{Model:saving_omhchorp:fig_flow_omhchorp} shows an overview of how vertical columns (modelled and measured) along with filters and related data are created in this thesis.
    As described in this chapter, satellite and modelled data are combined to form recalculated corrected vertical columns (VCC), with masks created for potential anthropogenic, pyrogenic, and smearing influence \added[id=tb]{(see Section \ref{Model:filter})}.
  ...
  \subsection{Creating new AMF using GEOS-Chem}
    ...
    \subsubsection{Updating shape factors}
      ...
      From Equation \ref{Model:AMF:eqn_AMF_intwSdz}\deleted[id=mh]{ we have}:
      $$ AMF = \int_0^\infty \omega(z) S(z) \mathrm{d}z $$
      Using the $\omega(z)$ from satellite data, along with \deleted[id=mh]{our }calculated $S_z$ interpolated linearly onto the same vertical grid as $\omega(z)$, the AMF can be determined through integration.
    ...
    \subsubsection{Recalculating the AMF}
      ...
      This allows both vertical column recalculation and reference sector correction.\deleted{.}
  ...
  \subsection{Vertical columns from AMF}
    ...
    All that remains for recalculating the total vertical column using \replaced[id=mh]{the}{our} new a priori shape factor is to apply the new $AMF$ to the slant columns and grid them onto \replaced[id=mh]{a}{our} chosen resolution.
  ...
  \subsection{Reference sector correction}
    ...
    To remove effects from the deterioration of the satellite instrument, measurements over the remote Pacific (\deleted[id=mh]{our }reference sector) are combined with GEOS-Chem simulations to create a reference sector correction (RSC).
    In this thesis\deleted[id=mh]{ we use} OMI HCHO columns \added[id=mh]{are used} to estimate isoprene emissions, and correcting background HCHO does not affect this calculation.
    ...
    % Visualisation of RSC referring to plot and 8-day averaged corrections
    ...
    \mypic{Figures/OMI_link/RSC_track_corrections20050101.png}{%
      Example of track correction interpolation for January 1st 2005.
      Points represent the difference between satellite slant column measurements and modelled slant columns over the remote Pacific.
  ...
  \subsection{Binning the results daily}
    ...
    %, as shown in Figure \ref{Model:saving_omhchorp:fig_flow_omhchorp}.
    ...
    \begin{enumerate}
      ...
      \item OMI corrected vertical column ($VCC_{OMI}$) %($\Omega_{GC}$,$\Omega_{OMI}$)
    ...
    This process requires processing time and storage space, and is performed on the National Computational Infrastructure (NCI) supercomputer.
  ...
  \label{Model:omiRecalc:diff}
    ...
    %up to 40\% underestimated \parencite[e.g.,][]{Zhu2016,DeSmedt2015,Barkley2013}.
    ...
    \mypic{Figures/OMI_link/VCC_check_200501.png}{%
      ...
      Row 2: same but for the monthly average for January 2005.
      Row 3: the distribution over the month for each of the three column amounts.
    ...
    \mypicw{0.75\textwidth}{Figures/OMI_link/N_pix_comparison.png}{%
      Monthly pixel counts over Australia (non-oceanic) used in recalculation of vertical columns.
      $N_{GC}$ refers to the number of pixels used in AMF recalculation without running PP code, $N_{PP}$ refers to those that have been recalculated using PP code.
    ...
    \mypic{Figures/OMI_link/VCC_distributions_2005.png}{%
      Top row: averaged OMI satellite VCC for 2005, from the OMHCHO dataset (left, $VCC_{OMI}$), recalculated using GEOS-Chem shape factors  (middle, $VCC_{GC}$), and recalculated using GEOS-Chem shape factors and scattering weights (right, $VCC_{PP}$).
      ...
      Oceanic pixels are filtered out, so only land pixels are included in this figure.
    }{\label{Model:omiRecalc:diff:fig_VCC_distributions}}
    
    
\deleted[id=tb]{Section 2.7 is moved into Chapter 3: Section 3.2.3}
  
\deleted[id=mh]{Section 2.8 is removed, text and figure moved to end of Section 2.6.1}
...
\section{Data access}
  ...
  Here\deleted[id=mh]{ I list} the datasets used in this thesis \added[id=mh]{are listed}, along with where \replaced{they are from and where they are used}{it is from and where it is used}.
  ...
  \begin{description}
    ...
    A summary can be found at \url{https://disc.gsfc.nasa.gov/datasets/OMAERUVd\_V003/summary}, \fullcite{Omar2008}.

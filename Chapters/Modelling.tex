...
\section{Measurement Techniques}
  ...
  \subsection{DOAS}
    ...
    \begin{equation*}
      I = I_0 \exp { \left( \Sigma_i \int \eta_i \alph_i ds \right) }
    \end{equation*}
  ...
  \label{Model:Meas:sat}
    ...
    Using equations \ref{Model:Meas:DOAS:eqn_beerslaw} and \ref{ch_HCHO:eqn:AMFratio} gives us an expression for the AMF which includes scattering:
...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Creating new shape factors}
    % What is the AMF -> leading to what is the shapefactor
    The shape factor (S) is one of the key components in creation of the AMF (see section \ref{Model:Meas:sat}).
    ...
    The AMF is determined using scattering weights and shape factors based on a priori estimates of HCHO vertical profiles.
    In this section I describe how the shape factor is calculated before being used to recalculate the AMF read from OMHCHO, following the method layed out initially in \textcite{Palmer2001}.
    ...
    The satellite shape factor is replaced using GEOS-Chem V10.01 overpass time simulated HCHO profiles.
    
    
    % Equations leading to shape factor
  ...
  \subsection{Scattering weight and the shape factor}
    ...
    \subsubsection{Normalised vertical shape factor}
      ...
      The vertical shape factor is normalised by the total vertical column.
      ...
      In order to normalise these vertical density profiles over the globe, we divide by the modelled total vertical column $\Omega_{HCHO}$ which is determined by:
      \begin{equation*}
    ...
    \subsubsection{Sigma coordinate conversion}
      ...
      Calculation of the vertical shape factor on the sigma coordinates S$_\sigma$ follows the method of \textcite{Palmer2001}.

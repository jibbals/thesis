...
\section{GEOS-Chem}
  ...
  \subsection{GEOS-Chem isoprene modelling}
    ...
    \subsubsection{Emissions from MEGAN}
      ...
      These moisture states are disabled in GEOS-Chem V10.01, which is partly because accurate maps of soil moisture are not available.
      
      In GEOS-Chem the isoprene emissions can be globally multiplied by a constant factor.
      By running the model with the biogenic isoprene emissions halved while other parameters remain unchanged, an estimate of modelled HCHO sensitivity to isoprene emissions can be determined.
      This, along with smearing sensitivity, is described in Section \ref{BioIsop:Methods:Smearing}.
...
\section{Recalculation of OMI HCHO}
  ...
  Additionally, OMAERUVd and OMNO2d data are used to create smoke and anthropogenic influence masks which are also stored in the OMHCHORP dataset.
  ...
  \subsection{Reference sector correction}
    ...
    The reference sector correction is independently calculated for each of the 60 tracks, at each latitude in the 500 bins described above.
...
\label{Model:Analysis}
  ...
  \subsection{Temperature vs HCHO}

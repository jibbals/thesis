...
\section{Recalculation of OMI HCHO}
  ...
  The end product is three sets of corrected vertical columns: the original, one using GEOS-Chem shape factors, and one from Palmer's code.
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item Total column HCHO from GEOS-Chem ($\Ogc$) is calculated $\Ogc = \Sigma_z \left(n_{HCHO}(z) \times H(z) \right)$, along with total column air ($\Omega_{A}$, calculated similarly)
      ...
      \item The original AMF, AMF$_{PP}$ (created separately), and AMF$_{GC}$ are used to determine the new vertical columns: $\Omega = SC/AMF$
  ...
  \subsection{Reading satellite data}
    ...
    The data set used is from the Ozone Monitoring Instrument (OMI) onboard the Aura satellite, as it has data for the entire time line and sufficiently covers the southern hemisphere.
  ...
  \subsection{Creating a new AMF using GEOS-Chem shape factors}
  ...
  \subsection{Reference sector correction}
    ...
    It uses the difference between slant columns ($\Os$) and reference slant columns ($\Omega_{S_0}$) divided by the AMF, plus the modelled reference sector column ($\Omega_{V_B}$):
    \begin{equation*}

...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Creating new AMF using GEOS-Chem}
    ...
    \subsubsection{Recalculating the AMF using PP code}
      ...
      I will refer to this as the PP code, $AMF_{PP}$ refers to AMF fully recalculated using this code, and $\Omega_{PP}$ are the vertical columns derived from these $AMF_{PP}$.
...
\section{Filtering Data}
  ...
  The main interference in the isoprene signal in HCHO comes from fire smoke plumes and major anthropogenic source regions.
  ...
  \textcite{Marais2012} removed pixels colocated with non zero fire counts in any of the prior eight days, within grid squares with 1\degr$\times 1$\degr resolution.
  ...
  %JENNY NOTE:Need to also comment here on the influence of the pyrogenic filter, and how this affects this overall (and comment on what the table shows). 
  ...
  \subsection{Fire and smoke}
    ...
    \begin{enumerate}
      ...
      \item Grid squares with AAOD over this threshold are considered potentially affected by transported fire smoke.
    \end{enumerate}
    ...
    Filtering fire smoke using AAOD is done by removing OMHCHO gridsquares where the AAOD is above a 0.03, after the AAOD is mapped from 1x1\degr to the same \highhr resolution as our OMHCHO gridded product.
    The threshold is determined through analysing AAOD over Australia in 4 scenarios: normal conditions, active local fires, during influence from transported fire smoke, and large scale dust storms.
    ...
    \subsubsection{Checking that fire masks are influencing pyrogenic HCHO}
      ...
      % JENNY COMMENT: I'm confused - is this the stuff you are showing in Figs. 2.42-2.44 that were discussed above? In general this whole section (although I'm suggesting it not be a subsection) needs to be re-worked. The order is funny and it is hard to understand.
  ...
  \subsection{NOx}
    ...
    OMNO2d from 2005 is used to determine a suitable threshold for anthropogenic influence by looking at NO$_2$ columns near several major cities in the south eastern sector of Australia.
  ...
  \section{Saving data}
    ...
    The output keeps the original AMF as well as those recalculated using GEOS-Chem.

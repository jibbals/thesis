...
\label{Model:Datasets}
  ...
  \subsection{Satellite}
    ...
    \subsubsection{OMHCHO}
      ...
      I use the NASA OMHCHOv003 data product \citep{Abad2015}, with HCHO determined using the spectral window $328.5$~nm$ - 356.5$~nm. 
      ...
      Slant columns range from $\sim 4\times 10^{15} $ to $\sim 6 \times 10^{16}$~molec cm$^{-2}$, with uncertainties from 30\% (larger columns) to over 100\% (smaller columns) \citep{Abad2015}.
    ...
    \subsubsection{OMAERUVd}
      ...
      This is due these particles scattering and absorbing UV radiation \citep{Ahn2008}.
      ...
      The product is most sensitive to error in the form of subpixel scale cloud interference, so I select AAOD as the basis for my smoke filter as it is least affected by clouds \citep{Ahn2008}.
...
\section{Measurement Techniques}
  ...
  \label{Model:Meas:sat}
    ...
    \subsubsection{Uncertainties}
      ...
      This bias has been measured as a 13\% positive monthly mean bias by \cite{Palmer2001, Surl2018}.
...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Calculating the apriori shape factor}
    ...
    Since we are using the $\omega$ provided by OMI, the AMF$_G$ term is removed from this calculation when compared against \cite{Palmer2001}.
    ...
    \subsubsection{Normalised vertical shape factor}
      ...
      The additional steps to calculate a normalised vertical shape factor S$_\sigma$ are first layed out in \cite{Palmer2001}.
...
\section{Filtering Data}
  ...
  Determining the AAOD due to smoke can be difficult since both smoke and dust absorb UV radiation \citep{Ahn2008,Marais2012}.
  AAOD is should be less sensitive to cloud contamination than AOD, and I use AAOD from the daily gridded level 3 satellite product OMAERUVd \citep{Ahn2008} described in section \ref{Model:Datasets:OMAERUVd} to provide a filter for smoke plumes.
  
  Filtering fire smoke using AAOD is done by removing OMHCHO gridsquares where the AAOD is above a threshhold, after the AAOD is mapped from 1x1\degr to the same 0.25x0.3125\degr resolution as our OMHCHO gridded product.
  ...
  Figure \ref{Model:Filter:fire:eqn_typicalAAOD} shows AAOD (columns 1 and 2), with AAOD distribution in column 3, along with satellite imagery on the same day in column 4 (from \url{https://worldview.earthdata.nasa.gov/}).
  The scenarios listed are shown from row 1 to 4, and AAOD greater than 0.03 is demarked by diagonal lines in column 1.
  ...
  \mypic{'Figures/OMI_link/typical_AAODs.png'}
    ...
    {\label{Model:Filter:fire:eqn_typicalAAOD}}

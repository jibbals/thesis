...
%      \end{figure}
      ...
      The importance of apriori and ancillary data (such as surface albedo and cloud top height) is also shown, as it sharply affects the structural uncertainty.
...
\section{Recalculation of OMI HCHO}
  ...
  The AMF is needed to transform the slant column (SC) viewed by the satellite into a vertical column (VC, or $\Omega$):
  \begin{equation}
  ...
  When comparing OMI VCs to GEOS-Chem, the satellite AMF needs to be recalculated using GEOS-Chem modelled vertical gas profiles as the aprioris.
  ...
  % Very brief outline
  Two new AMFs are calculated, both using GEOS-Chem HCHO profiles as the new apriori. 
  The first AMF (AMF$_GC$) uses the original satellite scattering weights while recalculating the shape factor, whereas the second AMF (AMF$_{PP}$) also recalculates scattering weights.
  ...
  A reference sector correction is determined using a similar method to that seen in \textcite{Abad2016}, (see section \ref{Model:omiRecalc:RSC}).
  This correction is unique for each of the 60 \textit{measurement tracks} used by OMI.
  Finally the correction is applied to each pixel to create the corrected vertical column (VCC). 
  The end product is three sets of VCCs: the original (VCC$_{OMI}$), VCC$_{GC}$, and VCC$_{PP}$.
  ...
  \subsection{Outline}
    ...
    Here is an outline in computational order of what takes place when recalculating the OMI HCHO VC.
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item Total column HCHO is calculated $\Omega_{HCHO} = \Sigma_z \left(N_{HCHO}(z) \times H(z) \right)$, along with total column air ($\Omega_{A}$, calculated similarly).
        ...
        \item S$_\sigma(z)$ is calculated on each altitude: $S_\sigma(z) = mr_{HCHO}(z) \times \Omega_A / \Omega_{HCHO}$
      \end{enumerate}
      \item Satellite pixels and associated scattering weights ($\omega(z)$), pressure levels, latitude and longitude are read from the OMHCHO dataset.
      \item For each pixel, a new AMF (AMF$_GC$)is created using the GEOS-Chem shape factors and satellite scattering weights.
      \begin{enumerate}
        \item $\omega$s are interpolated onto the same vertical dimensions (\textit{z} and $\sigma$) as the shape factors.
      ...
      \item The AMF$_{PP}$ (created separately) and AMF$_{GC}$ are used to determine the new vertical columns: $\Omega_{HCHO} = SC/AMF$.
      ...
      \begin{enumerate}
        \item GEOS-Chem satellite overpass output total columns from 140{\degr}W to 160{\degr}W are averaged longitudinally to provide a modelled reference sector.
        \item The modelled reference slant columns (MRSC) are calculated using $SC = VC \times AMF$ for each AMF.
        \item For each satellite pixel between 140{\degr}W and 160{\degr}W, the correction is calculated as the SC minus the MRSC interpolated linearly to the matching latitude.
        \item These corrections are binned by satellite track (1-60), and latitude (0.36\degr; 500 latitudes from 90{\degr}S to 90{\degr}N).
        \item The median entry of each bin is the RSC used to correct each satellite pixel corresponding to the matching satellite track and latitude.
      \end{enumerate}
      \item VCC are determined for each pixel using $VCC = (SC - RSC(lat,track) )/AMF$ for each AMF.
      \item The VCC are binned onto a 0.25\degr by 0.3125\degr grid along with how many pixels have been binned, and the average pixel uncertainty in product OMHCHORP.
    \end{enumerate}
  ...
  \subsection{Scattering weight and the shape factor}
    ...
    The equations and method of recalculating this apriori shape factor with the newer GEOS-Chem V10.01 code is layed out in this section.

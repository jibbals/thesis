...
\section{Calculating new AMF}
  ...
  The a priori is used when deriving the air mass factor (AMF), which is needed to transform satellite slant columns ($\Os$) into vertical columns ($\Omega$ in molecules cm$^{-2}$):
  The AMF is the ratio of $\Os$ to the vertical column
  \begin{equation} \label{Model:AMF:eqn_AMFFrac}
    AMF=\frac{\Os}{\Omega} = \frac{\tau_s}{\tau_v}
  \end{equation}
...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        ...
        \item The shape factor S$_z(z)$ is calculated on each altitude 
        \begin{equation*}
      ...
      \begin{enumerate}
        \item GEOS-Chem satellite overpass output ($\Ogc$ from 140{\degr}W to 160{\degr}W) are averaged monthly and longitudinally to provide modelled vertical columns over the reference sector.
        \item modelled vertical columns ($\Omega_{GEOS-Chem}$) are turned into modelled slant columns by multiplication with the AMF over the reference sector ($SC_{GEOS-Chem} = \Omega_{GEOS-Chem} \times AMF$).
        \item For each satellite pixel in the reference sector, a correction is calculated as the measured $SC$ minus the modelled slant column at the nearest latitude:
        \begin{equation*}
  ...
  \subsection{Creating new shape factors}
    ...
    \end{equation}
    where $\eta_{HCHO}$ is the number density of HCHO, $\eta_a$ is the number density of air (from model output), and ppb$_{HCHO}$ is the molecules of that species per billion molecules of air.
  ...
  \subsection{Creating new AMF using GEOS-Chem}
    ...
    \subsubsection{Saving the AMF with satellite pixels}
      ...
      Satellite pixels (see Section \ref{Model:datasets:OMHCHO}) are read and filtered for quality (see Section \ref{Model:omhcho:pixel_filtering}) and have their AMFs calculated as shown above. 
  ...
  \subsection{Vertical columns from AMF}
    ...
    Row two from left to right shows the relative differences between $\Omega_{PP}$ and $\Omega_{GC}$, $\Omega_{orig}$ and $\Omega_{GC}$, and $\Omega_{orig}$ and $\Omega_{PP}$.
    TODO: Add this plot here.
    
  \subsection{Reference sector correction}
  
    % JENNY COMMENT: I think this section needs a bit more justification/clarity for why it is ok to correct the satellite data using the modelled values. Basically I think you need to point out that at the end of the day you are only interested in the contribution to HCHO from NMVOC oxidation, and so adjusting the background (which is solely from CH4) does not introduce bias into the satellite data (right?).
    ...
    To get the modelled slant columns over the reference sector ($SC_{GEOS-Chem}$), each of the AMFs (calculated in prior sections) is applied to the modelled vertical columns using Equation \ref{Model:omiRecalc:eqn_AMFratio}: 
    \begin{equation*}
    ...
    Satellite measurements (pixels) over the remote Pacific are used to determine a correction array based on the difference between measured and modelled slant columns.

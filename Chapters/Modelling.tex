...
\section{Recalculation of OMI HCHO}
  ...
  \subsection{Outline}
    ...
    \begin{enumerate}
      ...
      \begin{enumerate}
        \item GEOS-Chem satellite overpass output ($\Ogc$ from 140{\degr}W to 160{\degr}W) are averaged monthly and longitudinally to provide a modelled reference sector $\Omega_0[lat]$.
  ...
  \subsection{Corrected vertical columns}
    ...
    Corrected vertical columns, or VCC, are created using the difference between slant columns ($\Os$) and reference slant columns ($\Omega_{S_0}$) divided by the AMF, plus the modelled reference TODO Slant column or sector column $\Omega_{V_B}$ or MRSC? 
    %TODO Check this and update this equation to make sense and use the right abbreviations:
    \begin{equation*}
    VCC = \frac{ \left( \Os - \Omega_{S_0} \right) }{ AMF } + \Omega_{V_B}
    \end{equation*}

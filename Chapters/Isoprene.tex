...
\label{ch_isop:sec:satelliteHCHO}
  ...
  \subsection{HCHO Vertical Column Calculation}
    ...
    \begin{equation*}
      \nabla I = \int_0^{\tau_v}{\frac{\delta I_B}{\delta \tau} \mathrm{d}\tau}
    \end{equation*}
    which can be placed into equation \ref{ch_isop:eqn:AMFthin}:
    \begin{equation*}
      AMF \approx \frac{-1}{\tau_v} \int_0^{\tau_v}{\frac{\delta \ln{I_B}}{\delta \tau} \mathrm{d}\tau}
    \end{equation*}
    and rewritten as:
    \begin{equation} \label{ch_isop:eqn:AMFcross}
      AMF = \frac{-1}{\tau_v} \int_0^\infty {\frac{\delta \ln{I_B}}{\delta \tau} \alpha(z)\eta(z)\mathrm{d}z}
    \end{equation}
    ...
    \begin{equation*}
      AMF=-\int_0^\infty{ \frac{\delta \ln{I_B}}{\delta \tau} \frac{\alpha(z)}{\alpha_e} \frac{\eta(z)}{\Omega_v} \mathrm{d}z }
    \end{equation*}
    ...
    \begin{equation}
      w(z) = -\frac{1}{AMF_G} \frac{\alpha(z)}{\alpha_e} \frac{\delta \ln{I_B}}{\delta \tau}
    \end{equation}
...
\label{ch_isop:sec:creatinginventory}
  ...
  \subsection{Regridding to x by x 8-day averaged vertical columns}
    ...
    Each satellite measurement of total vertical column has an associated shape factor, AMF, scattering weight, viewing zenith angle, solar zenith angle, and pacific region reference correction.
    ...
    The shape factors and scattering weights lie along a z-axis which is vertically resolved to 47 layers and is shown in figure TODO: make figure showing this stuff and an example profile.
    ...
    This is done in Python using the TODO:abc integration method.
  ...
  \subsection{Constructing vertical columns of HCHO}
    The vertical column scattering weights and apriori shape factors provided in the OMHCHO dataset are defined on 47 levels with units being TODO: check these.
    ...
    \begin{equation} \label{ch_isop:eqn:ppbto}
      \eta_{HCHO} = ppb_{HCHO} \eta_a 10^{-9}
    \end{equation}
    ...
    \begin{equation*}
      \Omega_{HCHO} = \Sigma_z \left( ppb_{HCHO}(z) (P(z)-P(z+1)) 2.12e13 \right)
    \end{equation*}
    ...
    $ \sqrt{ \left( P_1 * P_{surf} \right)} = P_0 $
    ...
    \begin{equation*}
      \Omega_a(i,j) = \Sigma_z \left( N_a(i,j,z) * H(i,j,z) \right)
      \Omega_z(i,j) = \Sigma_z \left( N_{HCHO}(i,j,z) * H(i,j,z) \right)
    \end{equation*}
    where $N_a$, and $N_{HCHO}$ are the densities of air and HCHO, H is the layer height (for each grid box).
    Note that HCHO density is determined from the outputted mixing ratio: $N_{HCHO} = C_{HCHO}*N_a$.
    ...
    As the GEOS-Chem V10.01 output is in bitpunch format, the code to read the data and create the shape factor is written in IDL, and is provided in supplementary TODO: put code into supplement section.
    ...
    This integral is applied in python by taking the sum of S$_\sigma(\sigma) \times \omega(\sigma) \times \mathrm{d}\sigma$ for each $\sigma$ determined at 72 levels in GEOS-Chem, with the provided $\omega$ linear interpolated to these same levels.
  ...
  \subsection{Determining and applying the pacific ocean reference sector normalisation}
    ...
    \begin{equation} \label{ch_isop:eqn:reference_sector_correction}
      Correction(i,j) = SC_{HCHO}(i,j) - VC_{GEOS-Chem}(lat(j)) * AMF(i,j)
    \end{equation}

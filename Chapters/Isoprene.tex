...
\section{Australian Biogenic Volatile Organic Compounds (BVOCs)}
  ...
  \subsection{Isoprene, Monoterpenes}
    ...
    The emissions of isoprene have been modelled at around 500~Tga$^{-1}$ using relatively few plant phenotypes to set global emission factors, in \citet{Guenther1995}.
  ...
  \subsection{Products and HCHO yield}
    ...
    \begin{tabular}{ | c | c | c | c |  }
      ...
      $\beta$-Pinene & 65$\pm$3 & 20$^\circ$~C, Low NO$_X$ & \citep{Lee2006}      \\ 
                            ...
                            & 45$\pm$8 & High NO$_X$ & \citep{AtkinsenArey2003}      \\ \hline
    \end{tabular}
...
\label{ch_isop:sec:satelliteHCHO}
  \subsection{Satellite Retrievals}
    ...
    The full method details for slant column retrieval by OMI are outlined in section \ref{ch_isop:sec:OMI_BOAS}.
    
    Uncertainty in a single pixel for OMI is quite high (todo: link to picture with one day average plus omi uncertainty).
    There are two methods of calculating this, one of which is used here and compared against the provided uncertainty (TODO) as shown in Section \ref{ch_isop:sec:uncertainty}.
    ...
    Random error can be reduced through temporal or spatial averaging, decreasing by a factor of $1/\sqrt{N}$ where N is the number of observations being averaged.
    Taking the eight day grided average with horizontal resolution of 0.25 by 0.3125 degrees (latitude by longitude) typically reduces the uncertainty from TODO: numbers.
    This can be seen more clearly in figure (TODO: image of OMI day retrieval compared to 8 day retrieval with uncertainties shown underneath).
    TODO: The figure shows both the provided and recalculated (using BIRA's method) uncertainty before and after averaging.
    
  \subsection{OMI Algorithm BOAS}
    \label{ch_isop:sec:OMI_BOAS}
    The following information comes from the OMHCHO dataset documentation at \citet{OMI_Readme} and \citet{Chance2002}.
  ...
  \subsection{HCHO Vertical Column Calculation}
    ...
    \begin{equation} \label{ch_isop:eqn:AKfromw}
      AK(z) = \frac{w(z)}{AMF}
    \end{equation}
    Note that this is an approximation based on the OMI satellite HCHO dataset as performed in \citet{Abad2015}.
    
  \subsection{Uncertainty in OMI total columns}
  \label{ch_isop:sec:uncertainty}
      ...
      (TODO: use this and compare my product uncertainty with provided uncertainty) \citep{DeSmedt2012,DeSmedt2015}.
...
\label{ch_isop:sec:creatinginventory}
  ...
  \subsection{Process Outline}
    ...
    This is done using AMF calculations as shown in section \ref{ch_isop:sec:satelliteHCHO:CalculationOfVC}.
  ...
  \label{ch_isop:sec:recalculating_AMF_description}
    ...
    Figure \ref{ch_isop:fig:AMF_Surface_Relevel} shows how these first two steps are applied using three fake array comparisons and updating the array with the lower surface level.
    ...
    \begin{figure}[!htbp]
      ...
      \label{ch_isop:fig:AMF_Surface_Relevel}
    \end{figure}
    ...
    As the GEOS-Chem V10.01 output is in bitpunch format, the code to read the data and create the shape factor is written in IDL, which has many procedures and functions which are alread written to handle reading this format.
  ...
  \subsection{Determining and applying the pacific ocean reference sector normalisation}
    ...
    Figure \ref{ch_isop:fig:Summary_RSC} the simulated reference sector VCs as an example, calculated on January 1st 2005.
    ...
    Another way to look at this correction is given in the OMI corrections panel of figure \ref{ch_isop:fig:Summary_RSC}, which has the sensors along the x axis, and latitude on the y axis, and shows how for this example 8-day period, the corrections are distributed with more negative values towards the left or right sensors, especially in the tropics.
  ...
  \subsection{Estimation of error or uncertainty}
    There are three main sources of error in the resulting HCHO columns: a) fitting error from the OMI retrieval, b) uncertainty in AMF calculations, and c) uncertainty of HCHO background.

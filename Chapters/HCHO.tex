...
\label{ch_HCHO:sec:satelliteHCHO}
  \subsection{Satellite Retrievals}
    ...
    There are two methods of calculating this, one of which is used here and compared against the provided uncertainty (TODO) as shown in Section \ref{ch_HCHO:sec:uncertainty}.
...
\label{ch_HCHO:sec:creatinginventory}
  ...
  \label{ch_HCHO:sec:recalculating_AMF_description}
    ...
    In order to reformulate the AMF, the 72 level profile from my GEOS-Chem UCX run is transformed from ppb to a normalized number density profile in order to match equation \ref{ch_HCHO:eqn:ShapeFactor}. 
    ...
    Once S$_\sigma(\sigma)$ is determined it is then stored in HDF-EOS5 format, to be used in conjunction with the satellite measurements to calculate an AMF as shown in equation \ref{ch_HCHO:eqn:AMFintwSdz}.
